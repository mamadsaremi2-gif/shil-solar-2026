...
// Case 1: 5 kWh/day off-grid villa, 2 autonomy days, LFP bank.
{
  ...
  assert.equal(pv.roughPanelCount, 3, 'PV rough panel count should be 3');
  assert.equal(pv.panelCount, 3, 'PV installed panel count should be 3');
  assert.equal(pv.panelSeriesCount, 3, 'stringing should use 3 panels in series');
  assert.equal(pv.panelParallelCount, 1, 'stringing should use one parallel string');
