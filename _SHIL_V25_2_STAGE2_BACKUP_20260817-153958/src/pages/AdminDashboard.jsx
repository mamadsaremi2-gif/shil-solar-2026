...
import "../styles/shil-admin-v21-comprehensive.css";
...
function compactProjectValue(value) {
  ...
  if (typeof value === "object") return value.title || value.name || value.model || value.status || `${Object.keys(value).length} مقدار`;
  return String(value);
}
...
function ProjectDetailBlock({ title, value }) {
  ...
  return <section className="shil-admin-project-report-section">
    <h4>{title}</h4>
    <div className="shil-admin-project-detail-grid">
      {entries.map(([key, entry]) => <div key={key}><span dir="auto">{key}</span><strong dir="auto">{compactProjectValue(entry)}</strong></div>)}
    </div>
    <details className="shil-admin-project-raw"><summary>جزئیات کامل این بخش</summary><pre dir="ltr">{JSON.stringify(value, null, 2)}</pre></details>
  </section>;
}
...
function ProjectAdminCard({ item, onDelete, onPublishScenario, notify }) {
  ...
  async function savePdf() {
    try {
      ...
      setShareOpen(false);
    } catch (error) { notify?.(error?.message || "ساخت PDF پروژه ناموفق بود."); }
  }
  ...
  async function sharePdf() {
    try {
      ...
      setShareOpen(false);
    } catch (error) { notify?.(error?.message || "اشتراک‌گذاری پروژه ناموفق بود."); }
  }
  ...
  return <article className={`shil-admin-project-card-v252 ${open ? "is-open" : ""}`}>
    ...
    {open ? <div className="shil-admin-project-card-body-v252">
      <div ref={reportRef} className="shil-admin-project-report-v252" dir="rtl">
        <header><h3>{title}</h3><p>{projectUserLabel(item)} · {route}</p><small>{updated ? new Date(updated).toLocaleString("en-US") : "بدون تاریخ"}</small></header>
        <div className="shil-admin-project-top-grid-v252">
          <div><span>وضعیت</span><strong>{item.status || "open"}</strong></div>
          <div><span>مسیر</span><strong>{route}</strong></div>
          <div><span>کاربر</span><strong>{projectUserLabel(item)}</strong></div>
          <div><span>Project ID</span><strong dir="ltr">{item.projectKey || item.id || "—"}</strong></div>
        </div>
...
function ConsumerEquipmentCms({ items, onChange, onSave, notify }) {
  ...
  return <div className="shil-admin-equipment-cms shil-admin-consumer-cms">
    ...
    <div className="shil-admin-cms-save"><button type="button" className="shil-primary-wide" onClick={async()=>{try{await onSave();notify?.("بانک مصرف‌کننده‌ها ذخیره و برای کاربران منتشر شد.");}catch(error){notify?.(error?.message||"ذخیره بانک مصرف‌کننده‌ها ناموفق بود.");}}}>ذخیره و انتشار برای کاربران</button></div>
  </div>;
}
...
async function setupOrRotateRecoveryCode() {
  ...
  async function publishProjectScenario(item) {
    try {
      ...
      patchRecord(item, { scenarioPublishedAt: new Date().toISOString(), scenarioId: scenario.id });
      notify(`پروژه به سناریوهای آماده ${projectDomain(item) === "emergency" ? "برق اضطراری" : "خورشیدی"} منتقل شد.`);
    } catch (error) {
      notify(error?.message || "انتقال پروژه به سناریوهای آماده ناموفق بود.");
    }
  }
...
{tab === "review" ? (
      ...
      {tab === "projects" ? (
        <AdminPanel title="مدیریت پروژه‌ها" subtitle="جدیدترین پروژه در ردیف اول است. هر پروژه را باز کنید تا جزئیات نهایی و عملیات مدیریتی نمایش داده شود.">
