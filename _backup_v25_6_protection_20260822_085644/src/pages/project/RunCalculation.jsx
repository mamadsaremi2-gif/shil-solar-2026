            ...
            <div className="shil-protection-detail-wrap">
              <table className="shil-protection-detail-table">
                <tbody>
                  ...
                  <tr><th>وضعیت تطبیق بانک</th><td>{bankMatched === true ? "تطبیق با بانک SHIL انجام شده" : bankMatched === false ? "ریتینگ محاسبه شده؛ تطبیق مدل کاتالوگی نهایی لازم است" : "وضعیت تطبیق بانک در داده ذخیره‌شده ثبت نشده"}</td></tr>
...
function buildExecutionContext({ domain, project, summary, result, solarDesign, systemSettings, methodSummary, methodKey, calculationInput, emergency }) {
  if (emergency) {
    ...
    const protection = firstNonEmptyObject(result?.protection, result?.values?.protection, design?.protection) || { protections: [], cables: [] };
...
function getProtectionRange(item = {}) {
  ...
  if (Array.isArray(raw)) {
    ...
    return nums.length ? [Math.min(...nums), Math.max(...nums)] : [0, Infinity];
  }
  const n = Number(raw);
  return Number.isFinite(n) && n > 0 ? [n, n] : [0, Infinity];
}

function selectProtectionBankItem(items = [], { group = "", types = [], side = "", currentA = 0 } = {}) {
  const wantedGroup = String(group || "").toLowerCase();
  const wantedSide = String(side || "").toLowerCase();
  const wantedTypes = (Array.isArray(types) ? types : [types]).map((x) => String(x || "").toLowerCase()).filter(Boolean);
  ...
  const matches = (Array.isArray(items) ? items : []).filter((item) => {
    ...
    const groupOk = !wantedGroup || itemGroup.includes(wantedGroup);
    const sideOk = !wantedSide || itemSide.includes(wantedSide);
    const typeOk = !wantedTypes.length || wantedTypes.some((type) => itemType.includes(type));
    if (!groupOk || !sideOk || !typeOk) return false;
    const [minA, maxA] = getProtectionRange(item);
    return !current || (current >= minA && current <= maxA) || current <= maxA;
  });
  if (!matches.length) return null;
  return matches.sort((a, b) => {
    const [, aMax] = getProtectionRange(a);
    const [, bMax] = getProtectionRange(b);
    return Math.abs(aMax - current) - Math.abs(bMax - current);
  })[0];
}

function asProtectionSelection(item, quantity = 1, design = {}) {
  const ratedCurrentA = Math.max(0, Number(design?.currentA || 0));
  const requiredVoltageV = Math.max(0, Number(design?.voltageV || 0));
  ...
  if (!item) {
    return {
      ...
      bankMatched: false,
      selectionReason: "ریتینگ مهندسی از خروجی محاسبات بازسازی شد؛ مدل دقیق کاتالوگی در داده ذخیره‌شده موجود نبود.",
    };
  }
  ...
  return {
    ...
    quantity: Math.max(1, Number(quantity || 1)),
    ratedCurrentA: ratedCurrentA || Number(item?.ratedCurrentA || item?.currentA || 0) || null,
    ratedVoltageV,
    requiredVoltageV: requiredVoltageV || null,
    polesRequired,
    catalogFamilyTitle: labelBase,
    ratedCurrentRangeA: Number.isFinite(maxA) ? [minA, maxA] : item?.ratedCurrentRangeA,
    standard,
    bankMatched: true,
    selectionReason: design?.reason || `ریتینگ ${ratedCurrentA || "موردنیاز"} آمپر با خانواده بانک SHIL تطبیق داده شد.`,
  };
}
...
function buildProtectionRows(ctx) {
  ...
  const bankProtectionItems = Array.isArray(protection?.protections) ? protection.protections : [];
  const emergencyDcRatingA = pick(protection?.batteryBreakerA, protection?.batteryFuseA, protection?.dcBreakerA, 0);
  const emergencyAcRatingA = pick(protection?.acBreakerA, protection?.acCurrentA, 0);
  const emergencyBatteryFuse = asProtectionSelection(
    selectProtectionBankItem(bankProtectionItems, { types: ["battery_fuse", "fuse"], currentA: emergencyDcRatingA }),
    ctx?.inverterCount,
    { currentA: emergencyDcRatingA, voltageV: pick(protection?.batteryVoltage, ctx?.dcVoltage, ctx?.batteryVoltage, 0), deviceType: "BATTERY_FUSE", standard: "IEC 60269", poles: "2P", label: `${emergencyDcRatingA || "-"} A Battery DC Fuse` }
  );
  const emergencyBatteryIsolator = asProtectionSelection(
    selectProtectionBankItem(bankProtectionItems, { types: ["battery_isolator", "isolator", "load_disconnector"], currentA: emergencyDcRatingA }),
    ctx?.inverterCount,
    { currentA: emergencyDcRatingA, voltageV: pick(protection?.batteryVoltage, ctx?.dcVoltage, ctx?.batteryVoltage, 0), deviceType: "DC_ISOLATOR", standard: "IEC 60947-3", poles: "2P", label: `${emergencyDcRatingA || "-"} A Battery DC Isolator` }
  );
  const emergencyBatteryBreaker = asProtectionSelection(
    selectProtectionBankItem(bankProtectionItems, { types: ["dc_mccb", "dc_mcb"], side: "battery", currentA: emergencyDcRatingA }),
    ctx?.inverterCount,
    { currentA: emergencyDcRatingA, voltageV: pick(protection?.batteryVoltage, ctx?.dcVoltage, ctx?.batteryVoltage, 0), deviceType: "DC_MCCB", standard: "IEC 60947-2", poles: "2P", label: `${emergencyDcRatingA || "-"} A Battery DC Breaker` }
  );
  const emergencyAcBreaker = asProtectionSelection(
    selectProtectionBankItem(bankProtectionItems, { group: "ac", types: ["ac_breaker", "ac_mccb", "mcb", "mccb"], side: "ac", currentA: emergencyAcRatingA }),
    ctx?.inverterCount,
    { currentA: emergencyAcRatingA, voltageV: pick(protection?.acVoltage, ctx?.voltageAC, 220), deviceType: "AC_BREAKER", standard: "IEC 60947-2", poles: pick(protection?.ac?.poles, ctx?.phaseAC === "three" ? "3P/4P" : "1P+N/2P"), label: `${emergencyAcRatingA || "-"} A AC Breaker` }
  );
  const emergencyAcSpd = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { group: "ac", types: ["spd"], side: "ac", currentA: 0 }), ctx?.inverterCount);
  const emergencyResidual = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { group: "ac", types: ["rcd", "rcbo"], side: "ac", currentA: emergencyAcRatingA }), ctx?.inverterCount);

  const batteryNested = protection?.batteryDc || {};
  const batteryFuseSelection = batteryNested?.fuseSelection || emergencyBatteryFuse;
  const batteryBreakerSelection = batteryNested?.breakerSelection || emergencyBatteryBreaker || emergencyBatteryIsolator;
  const batteryIsolatorSelection = batteryNested?.isolatorSelection || emergencyBatteryIsolator;
  const battery = {
    ...batteryNested,
    fuseSelection: batteryFuseSelection,
    breakerSelection: batteryBreakerSelection,
    isolatorSelection: batteryIsolatorSelection,
    fuse: pick(batteryFuseSelection?.label, batteryNested?.fuse, batteryNested?.fuseA ? `${formatMetric(batteryNested.fuseA, "A", 2)} FUSE DC` : null, protection?.batteryFuseA ? `${formatMetric(protection.batteryFuseA, "A", 2)} FUSE DC` : null, protection?.dcBreakerA ? `${formatMetric(protection.dcBreakerA, "A", 2)} Battery DC Fuse` : null, unifiedBattery?.fuse_A ? `${formatMetric(unifiedBattery.fuse_A, "A", 2)} ${cleanValue(unifiedBattery.fuse_type, "FUSE DC")}` : null, ""),
    isolator: pick(batteryBreakerSelection?.label, batteryIsolatorSelection?.label, batteryNested?.isolator, batteryNested?.breakerA ? `${formatMetric(batteryNested.breakerA, "A", 2)} ${cleanValue(batteryNested?.breakerType, "کلید DC")}` : null, protection?.batteryBreakerA ? `${formatMetric(protection.batteryBreakerA, "A", 2)} DC Breaker` : null, protection?.dcBreakerA ? `${formatMetric(protection.dcBreakerA, "A", 2)} Battery DC Breaker` : null, unifiedBattery?.DC_breaker_A ? `${formatMetric(unifiedBattery.DC_breaker_A, "A", 2)} ${cleanValue(unifiedBattery.DC_breaker_type, "DC Breaker")}` : null, ""),
    designVoltageV: pick(batteryNested?.designVoltageV, protection?.batteryVoltage, ctx?.dcVoltage, ctx?.batteryVoltage, "-"),
    currentA: pick(batteryNested?.currentA, batteryNested?.breakerA, protection?.batteryCurrentA, protection?.dcBreakerA, unifiedBattery?.I_bat_A, unifiedBattery?.battery_cable?.design_current_A, "-"),
  };

  const acNested = protection?.ac || {};
  const acBreakerSelection = acNested?.breakerSelection || emergencyAcBreaker;
  const acSpdSelection = acNested?.spdSelection || emergencyAcSpd;
  const residualSelection = acNested?.residualProtection || emergencyResidual;
  const ac = {
    ...acNested,
    breakerSelection: acBreakerSelection,
    spdSelection: acSpdSelection,
    residualProtection: residualSelection,
    breaker: pick(acBreakerSelection?.label, acNested?.breaker, acNested?.breakerA ? `${formatMetric(acNested.breakerA, "A", 2)} ${cleanValue(acNested?.breakerType, "کلید AC")}` : null, protection?.acBreakerA ? `${formatMetric(protection.acBreakerA, "A", 2)} MCB AC` : null, unifiedAc?.breaker_A ? `${formatMetric(unifiedAc.breaker_A, "A", 2)} ${cleanValue(unifiedAc.breaker_type, "MCB AC")}` : null, ""),
    spd: pick(acSpdSelection?.label, acNested?.spd, unifiedAc?.SPD_AC?.type, ""),
    residual: pick(residualSelection?.label, ""),
    poles: pick(acNested?.poles ? `${formatNumber(acNested.poles, 0)}P` : null, acBreakerSelection?.poles ? cleanValue(Array.isArray(acBreakerSelection.poles) ? acBreakerSelection.poles.join("/") : acBreakerSelection.poles) : null, unifiedAc?.poles ? `${formatNumber(unifiedAc.poles, 0)}P` : null, ctx?.phaseAC === "three" ? "3P/4P" : "1P+N/2P"),
    designVoltageV: pick(acNested?.designVoltageV, protection?.acVoltage, unifiedAc?.SPD_AC?.voltage_rating, ctx?.voltageAC, "-"),
    currentA: pick(acNested?.currentA, acNested?.breakerA, protection?.acCurrentA, protection?.acBreakerA, unifiedAc?.I_AC_A, unifiedAc?.AC_cable?.design_current_A, "-"),
  };
  ...
  const batteryItems = hasBattery ? [
    { label: "فیوز باتری", value: battery.fuse, selection: battery.fuseSelection, meta: selectionMeta(battery.fuseSelection, "IEC 60269") },
    { label: "کلید DC", value: battery.isolator, selection: battery.breakerSelection, meta: selectionMeta(battery.breakerSelection, "IEC 60947-2") },
    battery.isolatorSelection?.label ? { label: "ایزولاتور", value: battery.isolatorSelection.label, selection: battery.isolatorSelection, meta: selectionMeta(battery.isolatorSelection, "IEC 60947-3") } : null,
  ].filter(Boolean) : [];
  const acItems = [
    { label: "کلید AC", value: ac.breaker, selection: ac.breakerSelection, meta: selectionMeta(ac.breakerSelection, "IEC 60947-2") },
    { label: "SPD", value: ac.spd, selection: ac.spdSelection, meta: selectionMeta(ac.spdSelection, "IEC 61643-11") },
    { label: "RCD / RCBO", value: ac.residual, selection: ac.residualProtection, meta: [ac.poles, ac.residualProtection?.sensitivityMA ? `${formatNumber(ac.residualProtection.sensitivityMA, 0)} mA` : null].filter(Boolean).join(" / ") },
  ].filter((item) => item.value && item.value !== "-");
  ...
  if (hasBattery) {
    rows.push({
      ...
      items: batteryItems,
      note: joinSpecs(`${formatMetric(battery.designVoltageV, "VDC", 2)} · ${formatMetric(battery.currentA, "A", 2)}`, breakingText(battery.breakerSelection), standardText(battery.breakerSelection)),
    });
  }
  rows.push({
    label: "حفاظت AC",
    value: joinSpecs(ac.breaker, ac.spd, ac.residual, ac.poles),
    items: acItems,
    note: joinSpecs(`${formatMetric(ac.designVoltageV, "VAC", 2)} · ${formatMetric(ac.currentA, "A", 2)}`, breakingText(ac.breakerSelection), standardText(ac.breakerSelection)),
  });
  ...
  if (hasBattery) rows.push({ label: "کابل باتری", value: cableValue(cableDetails.battery, pick(cables.battery, protection?.batteryCable, protection?.dcCable, protection?.dcCableMm2 ? `${formatMetric(protection.dcCableMm2, "MM²", 2)} Battery/DC Cable` : null), unifiedBattery?.battery_cable), note: cableDetails.battery && Object.keys(cableDetails.battery).length ? cableNote(cableDetails.battery) : cableNote(unifiedBattery?.battery_cable) });
  rows.push({ label: "کابل AC", value: cableValue(cableDetails.ac, pick(cables.ac, protection?.acCable, protection?.acCableMm2 ? `${formatMetric(protection.acCableMm2, "MM²", 2)} AC Cable` : null), unifiedAc?.AC_cable), note: cableDetails.ac && Object.keys(cableDetails.ac).length ? cableNote(cableDetails.ac) : cableNote(unifiedAc?.AC_cable) });
  return rows;
}
