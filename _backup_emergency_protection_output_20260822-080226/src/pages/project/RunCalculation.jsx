...
function asProtectionSelection(item, quantity = 1) {
  if (!item) return null;
  return {
    ...item,
    label: item?.label || item?.title || item?.model || item?.engineeringClass || item?.id || "تجهیز حفاظتی",
...
function buildProtectionRows(ctx) {
  ...
  const emergencyBatteryFuse = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { group: "battery", types: ["battery_fuse", "fuse"], side: "battery", currentA: emergencyDcRatingA }), ctx?.inverterCount);
  const emergencyBatteryIsolator = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { group: "battery", types: ["battery_isolator", "isolator"], side: "battery", currentA: emergencyDcRatingA }), ctx?.inverterCount);
  const emergencyBatteryBreaker = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { types: ["dc_mccb", "dc_mcb"], side: "battery", currentA: emergencyDcRatingA }), ctx?.inverterCount);
  const emergencyAcBreaker = asProtectionSelection(selectProtectionBankItem(bankProtectionItems, { group: "ac", types: ["ac_breaker", "ac_mccb", "mcb", "mccb"], side: "ac", currentA: emergencyAcRatingA }), ctx?.inverterCount);
