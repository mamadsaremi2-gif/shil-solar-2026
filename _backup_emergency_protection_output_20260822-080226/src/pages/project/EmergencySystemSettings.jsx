...
function protectionSelection(item, quantity = 1) {
  if (!item) return null;
  return {
    ...item,
...
function buildEmergencyDesign({ handoff, backupHours, reserveFactor, dodPercent, inverterId, batteryId, manualMode, banks, cableLengthM = 10, lengthFactor = 1.15 }) {
  ...
  const acBreakerA = nextAmp(acCurrentA * 1.25);
  const dcCableMm2 = nextSize(calcSection(dcCurrentA, dcDropV));
  const acCableMm2 = nextSize(calcSection(acCurrentA, acDropV));
  const batteryFuseSelection = protectionSelection(chooseProtection(rawProtection.protections, { group: "battery", types: ["battery_fuse", "fuse"], side: "battery", currentA: dcBreakerA }));
  const batteryIsolatorSelection = protectionSelection(chooseProtection(rawProtection.protections, { group: "battery", types: ["battery_isolator", "isolator"], side: "battery", currentA: dcBreakerA }));
  const batteryBreakerSelection = protectionSelection(chooseProtection(rawProtection.protections, { types: ["dc_mccb", "dc_mcb"], side: "battery", currentA: dcBreakerA })) || batteryIsolatorSelection;
  const acBreakerSelection = protectionSelection(chooseProtection(rawProtection.protections, { group: "ac", types: ["ac_breaker", "ac_mccb", "mcb", "mccb"], side: "ac", currentA: acBreakerA }));
  const acSpdSelection = protectionSelection(chooseProtection(rawProtection.protections, { group: "ac", types: ["spd"], side: "ac" }));
  const residualProtection = protectionSelection(chooseProtection(rawProtection.protections, { group: "ac", types: ["rcd", "rcbo"], side: "ac", currentA: acBreakerA }));
  const protection = {
    ...rawProtection,
    source: "SHIL_EMERGENCY_PROTECTION_V25_5_FIX",
    effectiveLengthM,
    ...
    ac: {
      ...
      residualProtection,
      breaker: acBreakerSelection?.label || `${acBreakerA} A AC Breaker`,
      spd: acSpdSelection?.label || "SPD AC",
