...
function buildProtectionRows(ctx) {
  ...
  const battery = {
    ...
    currentA: pick(batteryNested?.operatingCurrentA, protection?.batteryCurrentA, batteryNested?.currentA, batteryNested?.breakerA, unifiedBattery?.I_bat_A, unifiedBattery?.battery_cable?.design_current_A, "-"),
  };
  ...
  const ac = {
    ...
    residual: acNested?.residualProtection?.sensitivityMA
      ? `RCBO Type ${cleanValue(acNested?.residualProtection?.rcdType || "A")} | ${formatNumber(acNested?.residualProtection?.ratedCurrentA || acNested?.breakerA || protection?.acBreakerA, 0)} A | ${formatNumber(acNested.residualProtection.sensitivityMA, 0)} mA`
      : pick(acNested?.residualProtection?.label, ""),
...
export default function RunCalculation() {
  ...
  const projectInfoRows = emergency ? [
    ...
    { label: "مدت پشتیبانی هدف", value: `ساعت ${runContext.backupHours}` },
  ...
  const projectEquipmentRows = emergency ? [
    ...
    { label: "تعداد اینورتر", value: `عدد ${runContext.inverterCount}` },
    ...
    { label: "آرایش بانک باتری", value: `سری ${runContext.batterySeriesCount} × موازی ${runContext.batteryParallelCount}` },
    { label: "تعداد کل باتری", value: `عدد ${runContext.batteryCount}` },
    ...
    { label: "زمان پشتیبانی واقعی", value: `ساعت ${runContext.runtimeHours}` },
  ] : [
    { label: "اینورتر خورشیدی", value: runContext.inverter?.title || runContext.inverter?.name || "ثبت نشده" },
    { label: "تعداد اینورتر خورشیدی", value: `عدد ${runContext.inverterCount}` },
    ...
    { label: "تعداد باتری", value: `عدد ${runContext.batteryCount}` },
