    ...
    <SettingsSection title={title} meta={kind === "battery" ? "بانک باتری" : "اینورتر"}>
      <div className="shil-summary-data" data-keep-card="true">
        <SettingsGrid rows={[
          ...
          [kind === "battery" ? "تعداد کل" : "تعداد اینورتر", kind === "battery" ? `عدد ${faNumber(activeItem?.count || 0)}` : `عدد ${faNumber(activeItem?.count || 1)}`],
...
export default function EmergencySystemSettings() {
  ...
  const emergencyBatteryDetailRows = [
    ...
    ["آرایش سری/موازی", `سری ${faNumber(design.battery.seriesCount || 0)} × موازی ${faNumber(design.battery.parallelCount || 0)}`],
    ["ولتاژ پک", `${faNumber(design.battery.packVoltage, 1)} V`],
    ["تعداد کل", `عدد ${faNumber(design.battery.count)}`],
    ["ظرفیت بانک", `${faNumber(design.battery.bankCapacityAh)} AH`],
    ["انرژی خام بانک", `${enNumber(design.battery.grossBankEnergyKWh, 2)} KWH`],
    ["زمان پشتیبانی", `ساعت ${enNumber(design.battery.runtimeHours, 2)}`],
  ];
  ...
  return (
    ...
    >
      ...
      <div id="shil-emergency-settings-root" className="shil-page-scroll shil-settings-summary-page shil-emergency-parity-page">
        <SettingsSection title="پارامترهای طراحی" meta={manualMode ? "ورود دستی تجهیزات فعال" : "انتخاب هوشمند فعال"}>
          <div className="shil-summary-data">
            <SettingsGrid editable rows={[
              ["ساعت مبنای تجهیزات", `ساعت ${EMERGENCY_BASE_LOAD_HOURS}`, null],
        ...
        <SettingsSection title="نتایج اصلی طراحی" meta="محاسبه زنده">
          <div className="shil-summary-data">
            <SettingsGrid rows={[
              ...
              ["پشتیبانی واقعی", `ساعت ${enNumber(design.battery.runtimeHours, 2)}`],
            ]} />
        ...
        <BankSelect kind="battery" title="بانک ذخیره‌ساز انرژی" value={batteryId} onChange={(v) => { setManualMode(true); setBatteryId(v); }} items={batteryOptions} selectedItem={design.battery} smartMeta={`سری ${faNumber(design.battery.seriesCount || 0)} × موازی ${faNumber(design.battery.parallelCount || 0)}`} detailRows={emergencyBatteryDetailRows} />
