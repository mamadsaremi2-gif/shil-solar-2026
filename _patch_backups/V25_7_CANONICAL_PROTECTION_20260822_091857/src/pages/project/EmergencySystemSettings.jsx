...
function buildEmergencyDesign({ handoff, backupHours, reserveFactor, dodPercent, inverterId, batteryId, manualMode, banks, cableLengthM = 10, lengthFactor = 1.15 }) {
  ...
  const acBreakerSelection = protectionSelection(
    chooseProtection(rawProtection.protections, { group: "ac", types: ["ac_breaker", "ac_mccb", "mcb", "mccb"], side: "ac", currentA: acBreakerA }),
    1,
    { currentA: acBreakerA, operatingCurrentA: acCurrentA, voltageV: voltageAC, designFactor: 1.25, deviceType: "AC_BREAKER", standard: "IEC 60947-2", poles: voltageAC >= 380 ? "3P/4P" : "1P+N/2P", label: `${acBreakerA} A AC Breaker` }
  );
