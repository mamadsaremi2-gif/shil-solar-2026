...
function chooseProtection(items = [], { group = "", types = [], side = "", currentA = 0, voltageV = 0 } = {}) {
  ...
  const candidates = (Array.isArray(items) ? items : []).filter((item) => {
    ...
    if (wantedSide.includes("battery") && requiredVoltage > 0 && maxV > Math.max(125, requiredVoltage * 4)) return false;
...
function buildEmergencyDesign({ handoff, backupHours, reserveFactor, dodPercent, inverterId, batteryId, manualMode, banks, cableLengthM = 10, lengthFactor = 1.15 }) {
  ...
  // The main Battery -> Inverter feeder is sized for the selected inverter, not the instantaneous load.
  // Prefer a catalog minimum DC input voltage when present. If the current bank does not publish it,
  // use a conservative 90% nominal-bus engineering floor and expose that value in the result.
  const inverterRatedPowerW = toNumber(selectedInverter?.ratedPowerW || selectedInverter?.powerW, designPowerW);
  const minBatteryVoltageV = toNumber(
    selectedInverter?.minBatteryVoltageV || selectedInverter?.batteryMinVoltageV || selectedInverter?.dcInputMinV,
    dcBusVoltage * 0.90
  );
  const dcCurrentA = minBatteryVoltageV > 0 ? inverterRatedPowerW / minBatteryVoltageV / inverterEfficiency : 0;
  ...
  const sizeForAmpacity = (current) => standardSizes.find((size) => (copperAmpacityA[size] || 0) >= current) || standardSizes[standardSizes.length - 1];
  const largerSize = (a, b) => standardSizes[Math.max(standardSizes.indexOf(a), standardSizes.indexOf(b))];
  // IEC coordination invariant used by the runtime: Ib <= In <= Iz.
  // Sizing against dcBreakerA guarantees the selected cable ampacity Iz is not below breaker In.
  const dcCableMm2 = largerSize(nextSize(calcSection(dcCurrentA, dcDropV)), sizeForAmpacity(dcBreakerA));
  const dcCableAmpacityA = copperAmpacityA[dcCableMm2] || 0;
  const dcCoordinationPass = dcCurrentA <= dcBreakerA && dcBreakerA <= dcCableAmpacityA;
  const acCableMm2 = largerSize(nextSize(calcSection(acCurrentA, acDropV)), sizeForAmpacity(acBreakerA));
  const batteryFuseSelection = protectionSelection(
    chooseProtection(rawProtection.protections, { types: ["battery_fuse", "fuse"], currentA: dcBreakerA, voltageV: dcBusVoltage }),
    1,
  ...
  const batteryIsolatorSelection = protectionSelection(
    chooseProtection(rawProtection.protections, { types: ["battery_isolator", "isolator", "load_disconnector"], currentA: dcBreakerA, voltageV: dcBusVoltage }),
    1,
  ...
  const batteryBreakerSelection = protectionSelection(
    chooseProtection(rawProtection.protections, { types: ["dc_mccb", "dc_mcb"], side: "battery", currentA: dcBreakerA, voltageV: dcBusVoltage }),
    1,
  ...
  const residualProtectionBase = protectionSelection(
      ...
      1,
      { currentA: acBreakerA, operatingCurrentA: acCurrentA, voltageV: voltageAC, deviceType: "RCBO", standard: "IEC 61009-1", poles: voltageAC >= 380 ? "4P" : "1P+N/2P", label: `${acBreakerA} A / 30mA RCBO Type A` }
    );
  const residualProtection = {
    ...residualProtectionBase,
    label: `${acBreakerA} A / 30mA RCBO Type A`,
  ...
  const protection = {
    ...
    cableDetails: {
      battery: { areaMm2: dcCableMm2, currentA: dcCurrentA, ampacityA: dcCableAmpacityA, breakerA: dcBreakerA, coordinationPass: dcCoordinationPass, minDesignVoltageV: minBatteryVoltageV, lengthM: effectiveLengthM, voltageDropPercent: 2, label: `${dcCableMm2} mm² Battery/DC Cable` },
