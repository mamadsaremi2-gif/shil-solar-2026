...
export function normalizePersianNumber(value) { return String(value ?? "").replace(/[۰-۹]/g, (d) => "۰۱۲۳۴۵۶۷۸۹".indexOf(d)).replace(/[٠-٩]/g, (d) => "٠١٢٣٤٥٦٧٨٩".indexOf(d)).replace(/٫/g, ".").replace(/٬/g, "").replace(/,/g, "").trim(); }
export function toNumber(value, fallback = 0) { const number = Number(normalizePersianNumber(value)); return Number.isFinite(number) ? number : fallback; }
export function clampNumber(value, min, max, fallback = min) { const number = toNumber(value, fallback); return Math.max(min, Math.min(max, number)); }
export function normalizeAzimuthDeg(value, fallback = DEFAULT_SOLAR_AZIMUTH_DEG) { const n = toNumber(value, fallback); return ((n % 360) + 360) % 360; }
export function estimateRecommendedTilt(latitude) { const lat = Math.abs(toNumber(latitude, DEFAULT_PANEL_TILT_FALLBACK_DEG)); if (!Number.isFinite(lat)) return DEFAULT_PANEL_TILT_FALLBACK_DEG; return Math.round(clampNumber(lat * 0.9, 15, 45, DEFAULT_PANEL_TILT_FALLBACK_DEG)); }
export function estimateThermalDeratePercent(maxTemperatureC) { const maxTemp = toNumber(maxTemperatureC, 40); if (maxTemp <= 35) return 0; if (maxTemp <= 42) return 2; if (maxTemp <= 47) return 4; return 6; }
export function getAzimuthDeltaFromSouth(azimuthDeg) { const az = normalizeAzimuthDeg(azimuthDeg, DEFAULT_SOLAR_AZIMUTH_DEG); const raw = Math.abs(az - DEFAULT_SOLAR_AZIMUTH_DEG); return Math.min(raw, 360 - raw); }
export function estimateOrientationLossPercent(azimuthDeg) { const delta = getAzimuthDeltaFromSouth(azimuthDeg); if (delta <= 10) return 0; if (delta <= 25) return 2; if (delta <= 45) return 5; if (delta <= 70) return 10; if (delta <= 100) return 18; if (delta <= 140) return 28; return 38; }
export function estimateTiltLossPercent(tiltDeg, recommendedTiltDeg) { const tilt = clampNumber(tiltDeg, 0, 80, recommendedTiltDeg || DEFAULT_PANEL_TILT_FALLBACK_DEG); const recommended = clampNumber(recommendedTiltDeg, 0, 80, DEFAULT_PANEL_TILT_FALLBACK_DEG); const delta = Math.abs(tilt - recommended); if (delta <= 5) return 0; if (delta <= 10) return 1.5; if (delta <= 18) return 4; if (delta <= 30) return 8; return 14; }
export function getOrientationEfficiency({ azimuthDeg, tiltDeg, recommendedTiltDeg }) { const orientationLossPercent = estimateOrientationLossPercent(azimuthDeg); const tiltLossPercent = estimateTiltLossPercent(tiltDeg, recommendedTiltDeg); const totalOrientationLossPercent = clampNumber(orientationLossPercent + tiltLossPercent, 0, 55, 0); const orientationEfficiency = clampNumber(1 - totalOrientationLossPercent / 100, 0.45, 1, 1); return { orientationLossPercent, tiltLossPercent, totalOrientationLossPercent, orientationEfficiency }; }
export function estimateCorrosionRisk({ humidity, installType }) { const h = toNumber(humidity, 30); if (installType === "coastal" || h >= 70) return "high"; if (installType === "industrial" || h >= 55) return "medium"; return "low"; }
