...
export default function CalculationInputs() {
  ...
  const equipmentStats = React.useMemo(() => {
    ...
    return selectedItems.reduce((acc, item) => {
      ...
      const dailyEnergyKWh = (powerW * qty * hours) / 1000;
      acc.surgePowerW += powerW * qty * startFactor;
      acc.peakDailyEnergyKWh = Math.max(acc.peakDailyEnergyKWh, dailyEnergyKWh);
      if (isMotor) acc.motorCount += qty;
      if (hasSoftStarter && isMotor) acc.softStarterCount += qty;
      return acc;
    }, { selectedCount: 0, totalPowerW: 0, totalEnergyKWh: 0, peakDailyEnergyKWh: 0, acCurrentA: 0, startCurrentA: 0, surgePowerW: 0, motorCount: 0, softStarterCount: 0 });
      ...
      <div className="shil-equipment-page">
        ...
        {method === "equipment" ? (
          <section className="shil-env-card shil-equipment-results-card">
            <h3 className="shil-section-title">چکیده لیست تجهیزات</h3>
            <div className="shil-summary-grid">
              <div><span>تعداد تجهیزات انتخاب شده</span><strong>{enValue(selectedItems.length, "تجهیز")}</strong></div>
              <div><span>توان کل تجهیزات مصرفی</span><strong>{enValue(equipmentStats.totalPowerW || enginePreview.totalPowerW, "W")}</strong></div>
              <div><span>پیک توان</span><strong>{enValue(equipmentStats.surgePowerW || enginePreview.surgePowerW, "W")}</strong></div>
              <div><span>جریان</span><strong>{enValue(equipmentStats.acCurrentA || enginePreview.acCurrentA, "A", 2)}</strong></div>
              <div><span>پیک جریان</span><strong>{enValue(equipmentStats.startCurrentA || enginePreview.startCurrentA, "A", 2)}</strong></div>
              <div><span>انرژی مصرفی روزانه</span><strong>{enValue(equipmentStats.totalEnergyKWh || enginePreview.totalEnergyKWh, "kWh", 2)}</strong></div>
              <div><span>پیک انرژی مصرفی روزانه</span><strong>{enValue(equipmentStats.peakDailyEnergyKWh, "kWh", 2)}</strong></div>
              <div><span>مجموعه تجهیزات موتوری</span><strong>{enValue(equipmentStats.motorCount, "تجهیز")}</strong></div>
              <div><span>تجهیزات موتوری دارای سافت استارتر</span><strong>{enValue(equipmentStats.softStarterCount, "تجهیز")}</strong></div>
            </div>
            {selectedItems.length ? (
              <p className="shil-muted-note">این چکیده قبل از تأیید مرحله نمایش داده می‌شود و از تجهیزات انتخاب‌شده، تعداد، ساعت مصرف، جریان، پیک راه‌اندازی و وضعیت سافت‌استارتر محاسبه شده است.</p>
            ) : (
              <div className="shil-empty-selection">برای تکمیل چکیده، ابتدا حداقل یک تجهیز انتخاب کنید.</div>
            )}
          </section>
        ) : null}
