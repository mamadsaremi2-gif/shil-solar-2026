...
export default function CalculationInputs() {
  ...
  const [manualVoltage, setManualVoltage] = React.useState("230");
  const [manualHours, setManualHours] = React.useState(domain === "emergency" ? "6" : "5");
  const [manualIncludesStartup, setManualIncludesStartup] = React.useState("no");
  const [profileVoltage, setProfileVoltage] = React.useState("230");
  ...
  const STANDARD_STARTUP_FACTOR = 1.25;
  const manualVoltageBase = toNumber(manualVoltage || 230, 230);
  const manualVoltageFactor = manualVoltageBase >= 380 ? Math.sqrt(3) : 1;
  const startupAlreadyIncluded = manualIncludesStartup === "yes";
  const manualPowerBaseW = toNumber(manualPowerW, 0);
  const manualCurrentBaseA = toNumber(manualCurrentA, 0);
  const currentPowerBaseW = Math.round(manualCurrentBaseA * manualVoltageBase * manualVoltageFactor);
  const energyHoursBase = Math.max(1, toNumber(manualHours || 1, 1));
  const energyPowerBaseW = Math.round((toNumber(manualEnergyKWh, 0) * 1000) / energyHoursBase);
  const manualStartupFactor = startupAlreadyIncluded ? 1 : STANDARD_STARTUP_FACTOR;
  const manualPowerFinalW = Math.round(manualPowerBaseW * manualStartupFactor);
  const currentPowerFinalW = Math.round(currentPowerBaseW * manualStartupFactor);
  const energyPowerFinalW = Math.round(energyPowerBaseW * manualStartupFactor);
  const profilePowerFinalW = Math.round(profilePeakPowerW * (startupAlreadyIncluded ? 1 : Math.max(1, toNumber(profileStartFactor, 1.6))));

  const enginePreview = React.useMemo(() => {
    const voltage = toNumber(method === "solar_panel_power" ? acVoltageRoute : method === "profile" ? profileVoltage : manualVoltage || 230, 230);
    const currentVoltageFactor = voltage >= 380 ? Math.sqrt(3) : 1;
    const powerFromCurrent = method === "current" && manualCurrentA ? currentPowerFinalW : 0;
    ...
    const powerFromManual = method === "power" && manualPowerW ? manualPowerFinalW : method === "energy" ? energyPowerFinalW : method === "profile" ? profilePowerFinalW : 0;
  ...
  const manualVoltageNumber = toNumber(manualVoltage || 230, 230);
  const manualPhaseLabel = manualVoltageNumber >= 380 ? "۳۸۰ ولت سه‌فاز" : "۲۳۰ ولت تک‌فاز";
  const currentDerivedPowerW = currentPowerFinalW;
  const startupAnswerLabel = startupAlreadyIncluded ? "بله، در عدد واردشده لحاظ شده" : `خیر، ضریب استاندارد ${STANDARD_STARTUP_FACTOR}× اعمال شد`;
  ...
  const selectedEquipmentStats = React.useMemo(() => {
    const voltage = toNumber(manualVoltage || 230, 230);
    const phaseFactor = voltage >= 380 ? Math.sqrt(3) : 1;

    const isMotorEquipment = (item) => {
      const text = `${item.title || ""} ${item.category || ""} ${item.type || ""}`.toLowerCase();
      return Boolean(
        item.isMotor ||
        item.type === "inductive" ||
        Number(item.startupFactor || item.surgeFactor || 1) > 1.7 ||
        /موتور|پمپ|کمپرسور|فن|کولر|آسانسور|motor|pump|compressor|fan/.test(text)
      );
    };

    return selectedItems.reduce((acc, item) => {
      const quantity = Math.max(1, toNumber(item.quantity, 1));
      const powerW = toNumber(item.ratedPowerW || item.powerW, 0);
      const hours = Math.max(0, toNumber(item.usageHoursPerDay, 0));
      const isMotor = isMotorEquipment(item);
      const hasSoftStarter = Boolean(item.hasSoftStarter);
      const pf = Math.max(0.1, toNumber(item.powerFactor, isMotor ? 0.82 : 0.95));
      const baseCurrent = voltage > 0 ? (powerW * quantity) / (voltage * phaseFactor * pf) : 0;
      const startFactor = isMotor ? (hasSoftStarter ? 1.2 : 2.5) : 1;

      acc.selectedCount += quantity;
      acc.totalPowerW += powerW * quantity;
      acc.totalEnergyKWh += (powerW * quantity * hours) / 1000;
      acc.acCurrentA += baseCurrent;
      acc.startCurrentA += baseCurrent * startFactor;
      acc.surgePowerW += powerW * quantity * startFactor;

      if (isMotor) acc.motorCount += quantity;
      if (hasSoftStarter) acc.softStarterCount += quantity;
  ...
  }, [selectedItems, manualVoltage]);

  const finalPreview = React.useMemo(() => {
    if (method !== "equipment" || !selectedItems.length) return enginePreview;
    return {
      ...enginePreview,
      selectedCount: selectedEquipmentStats.selectedCount,
      totalPowerW: Math.round(selectedEquipmentStats.totalPowerW),
      totalEnergyKWh: Number(selectedEquipmentStats.totalEnergyKWh.toFixed(2)),
      acCurrentA: Number(selectedEquipmentStats.acCurrentA.toFixed(2)),
      startCurrentA: Number(selectedEquipmentStats.startCurrentA.toFixed(2)),
      surgePowerW: Math.round(selectedEquipmentStats.surgePowerW),
      motorCount: selectedEquipmentStats.motorCount,
      softStarterCount: selectedEquipmentStats.softStarterCount,
    };
  }, [method, selectedItems.length, enginePreview, selectedEquipmentStats]);
  ...
  const confirmLoad = () => {
    ...
    const result = persistLoadEngineResult({
      ...
      environmentAssessment,
      selectedItems,
      voltageAC: toNumber(method === "solar_panel_power" ? acVoltageRoute : method === "profile" ? profileVoltage : manualVoltage || 230, 230),
      phaseAC: toNumber(method === "solar_panel_power" ? acVoltageRoute : method === "profile" ? profileVoltage : manualVoltage || 230, 230) >= 380 ? "three" : "single",
    ...
    localStorage.setItem("shil:registeredCalculationParameters", JSON.stringify({
      method,
      methodTitle: contextMethodLabel,
      voltageAC: method === "profile" ? toNumber(profileVoltage, 230) : manualVoltageNumber,
      phase: method === "profile" ? (toNumber(profileVoltage, 230) >= 380 ? "سه‌فاز" : "تک‌فاز") : manualPhaseLabel,
      startupIncludedByUser: startupAlreadyIncluded,
      startupRule: startupAnswerLabel,
      totalPowerW: finalPreview.totalPowerW || currentPowerFinalW || manualPowerFinalW || energyPowerFinalW || profilePowerFinalW || 0,
      totalEnergyKWh: finalPreview.totalEnergyKWh || (toNumber(manualEnergyKWh, 0)) || Number((profileTotalEnergyWh / 1000).toFixed(2)) || 0,
      acCurrentA: finalPreview.acCurrentA || (manualVoltageNumber > 0 ? Number(((currentPowerFinalW || manualPowerFinalW || energyPowerFinalW || profilePowerFinalW || 0) / (manualVoltageNumber * manualVoltageFactor)).toFixed(2)) : 0),
      startCurrentA: finalPreview.startCurrentA || 0,
      surgePowerW: finalPreview.surgePowerW || currentPowerFinalW || manualPowerFinalW || energyPowerFinalW || profilePowerFinalW || 0,
      motorCount: selectedEquipmentStats.motorCount,
      softStarterCount: selectedEquipmentStats.softStarterCount,
    }));
      ...
      <div className="shil-equipment-page">
            ...
            {method === "profile" ? (
              <>
                <div className="shil-form-grid">
                  ...
                  <label>ولتاژ AC<select className="shil-input" value={profileVoltage} onChange={(e) => setProfileVoltage(e.target.value)}><option value="230">۲۳۰ ولت تک‌فاز</option><option value="380">۳۸۰ ولت سه‌فاز</option></select></label>
              ...
              <>
                <div className="shil-form-grid">
                  ...
                  <label>مسیر خروجی AC<select className="shil-input" value={acVoltageRoute} onChange={(e) => setAcVoltageRoute(e.target.value)}><option value="230">۲۳۰ ولت تک‌فاز</option><option value="380">۳۸۰ ولت سه‌فاز</option></select></label>
                </div>
              ...
              <div className="shil-form-grid">
                ...
                {method === "power" ? <label>توان کلی مصرف‌کننده W<input className="shil-input" value={manualPowerW} onChange={(e) => setManualPowerW(e.target.value)} placeholder="مثلاً 3500" inputMode="numeric" /></label> : null}
                {method === "current" ? <label>جریان کلی مصرف‌کننده A<input className="shil-input" value={manualCurrentA} onChange={(e) => setManualCurrentA(e.target.value)} placeholder="مثلاً 16" inputMode="decimal" /></label> : null}
                {(method === "power" || method === "current") ? (
                  <label>ولتاژ شبکه<select className="shil-input" value={manualVoltage} onChange={(e) => setManualVoltage(e.target.value)}><option value="230">۲۳۰ ولت تک‌فاز</option><option value="380">۳۸۰ ولت سه‌فاز</option></select></label>
                ) : (
                ...
                {["power", "current", "energy", "profile"].includes(method) ? <label>آیا مقدار واردشده با محاسبه جریان راه‌اندازی است؟<select className="shil-input" value={manualIncludesStartup} onChange={(e) => setManualIncludesStartup(e.target.value)}><option value="yes">بله</option><option value="no">خیر، ضریب استاندارد اعمال شود</option></select></label> : <label>ساعت استفاده / زمان برق اضطراری مورد نظر<input className="shil-input" value={manualHours} onChange={(e) => setManualHours(e.target.value)} inputMode="decimal" /></label>}
              </div>
            )}
          </section>
        ) : null}
        ...
        <section className="shil-env-card">
          <h3 className="shil-section-title">
            پارامترهای ثبت شده {methodResultTitle}
          </h3>
          <div className="shil-summary-grid">
            {method === "equipment" ? <div><span>تجهیزات انتخاب‌شده</span><strong>{selectedEquipmentTitles || "ثبت نشده"}</strong></div> : null}
            {method !== "power" && method !== "current" ? <div><span>تعداد تجهیزات</span><strong>{finalPreview.selectedCount || "ثبت نشده"}</strong></div> : null}
            {method !== "current" ? <div><span>توان کل</span><strong>{finalPreview.totalPowerW} W</strong></div> : null}
            {method !== "current" ? <div><span>انرژی روزانه</span><strong>{finalPreview.totalEnergyKWh} kWh</strong></div> : null}
            {method === "current" ? <div><span>جریان کل واردشده</span><strong>{toNumber(manualCurrentA, 0)} A</strong></div> : <div><span>جریان AC</span><strong>{finalPreview.acCurrentA} A</strong></div>}
            {method === "current" ? <div><span>ولتاژ شبکه</span><strong>{manualPhaseLabel}</strong></div> : null}
            {["power", "current", "energy", "profile"].includes(method) ? <div><span>وضعیت جریان راه‌اندازی</span><strong>{startupAnswerLabel}</strong></div> : null}
            {method === "current" ? <div><span>توان محاسبه‌شده نهایی</span><strong>{currentDerivedPowerW} W</strong></div> : null}
            {method !== "power" && method !== "current" ? <div><span>جریان راه‌اندازی</span><strong>{finalPreview.startCurrentA} A</strong></div> : null}
            {method !== "power" && method !== "current" ? <div><span>پیک استارت</span><strong>{finalPreview.surgePowerW} W</strong></div> : null}
            {method === "equipment" ? <div><span>تعداد تجهیزات موتوری</span><strong>{selectedItems.length ? (finalPreview.motorCount || "ندارد") : "ثبت نشده"}</strong></div> : null}
            {method === "equipment" ? <div><span>تعداد تجهیزات سافت‌استارتر دار</span><strong>{selectedItems.length ? (finalPreview.softStarterCount || "ندارد") : "ثبت نشده"}</strong></div> : null}
          </div>
