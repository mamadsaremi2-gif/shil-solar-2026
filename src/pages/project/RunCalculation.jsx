...
export default function RunCalculation() {
  ...
  function saveFinalProject() {
    ...
    upsertUserRecord("shil-projects", (item) => item.projectKey === projectKey, {
      projectKey, title: projectTitle, status: "final", domain: emergency ? "emergency" : "solar", currentStep: "run", resumeUrl: `/new-project/run/${domain}`,
      finalOutput: { project, summary, result, aiPreview, completedAt: new Date().toISOString() }
    });
    localStorage.setItem("shil:finalEngineeringOutput", JSON.stringify({ domain, project, summary, result, aiPreview, savedAt: new Date().toISOString() }));
    setRan(true);
  }

  return (
    <EngineeringPageShell title="اجرای محاسبات و خروجی نهایی">
      <section className="shil-card-stack shil-final-delivery-page">
        <div className="shil-section-card shil-delivery-hero">
          ...
          <div className="shil-summary-grid">
            ...
            <Row label="قابلیت خروجی" value="تصویر / PDF / ذخیره پروژه" />
          </div>
        </div>
        ...
        <div className="shil-section-card">
          ...
          <div className="shil-output-actions"><button type="button">ذخیره تصویر پروژه</button><button type="button">خروجی PDF مهندسی</button><button type="button">ذخیره پروژه</button><button type="button">اشتراک طراحی</button></div>
        </div>
