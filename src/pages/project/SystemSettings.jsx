...
function MethodSummaryCard({ handoff }) {
  ...
  return <section className="shil-section-card shil-config-block">
    ...
    <ShilWarningOverlay messages={summary.warnings} inline />
  </section>;
}
    ...
    <table className="shil-equipment-bank-table">
      ...
      <tbody>
        <tr>
          ...
          <td><bdi dir="ltr">{compactEquipmentLabel(activeItem, kind)}</bdi></td>
          <td><bdi dir="ltr">{qtyValue}</bdi></td>
          <td>
    ...
    <div className={open ? "shil-equipment-bank-details open" : "shil-equipment-bank-details"}>
      <div className="shil-equipment-bank-old-content">
        ...
        <div className="shil-bank-smart-note">
          ...
          <strong><bdi dir="ltr">{smartTitle}</bdi></strong>
          <small><bdi dir="ltr">{smartValue}</bdi></small>
        </div>

        <div className="shil-summary-grid shil-bank-datasheet-grid">
          {detailRows.filter(Boolean).map(([label, val]) => <div key={label}><span>{label}</span><strong><bdi dir="ltr">{val || "-"}</bdi></strong></div>)}
        </div>
      </div>
    </div>
  </section>;
...
export default function SystemSettings() {
  ...
  return <EngineeringPageShell title="تنظیمات" activeStep="system" backTo="/new-project/inputs/solar">
    ...
    <div id="shil-system-settings-unified-root" className="shil-page-scroll shil-system-settings-page" style={{ paddingBottom: "0px" }}>
