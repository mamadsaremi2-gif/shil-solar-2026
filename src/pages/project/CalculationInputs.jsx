...
import { CONSUMER_CATEGORY_ORDER, getConsumerEquipmentLibrary, inferConsumerCategory, loadConsumerEquipmentLibrary, searchConsumerEquipment } from "../../data/catalogs/consumerEquipmentLibrary.js";
import { buildScenarioCalculationInput } from "../../core/scenario/scenarioToEngineeringForm.js";
import { EMERGENCY_BASE_LOAD_HOURS, EMERGENCY_DEFAULT_BACKUP_HOURS, EMERGENCY_MIN_BACKUP_HOURS, EMERGENCY_MAX_BACKUP_HOURS, clampEmergencyBackupHours } from "../../core/calculation/emergencySizingRules.js";
...
function buildEquipmentTrace(item = {}, override = {}, options = {}) {
  ...
  const sourceUsageHoursPerDay = Math.max(0, toNumber(override.usageHoursPerDay ?? item.usageHoursPerDay ?? item.defaultHours ?? 0, 0));
  ...
  const backupHours = domain === "emergency" ? clampEmergencyBackupHours(options.backupHours) : 0;
  const effectiveEnergyHours = domain === "emergency" ? EMERGENCY_BASE_LOAD_HOURS : usageHoursPerDay;
  const energyWh = effectivePowerW * effectiveEnergyHours;
  const backupEnergyWh = domain === "emergency" ? effectivePowerW * backupHours : energyWh;
  return {
    id: item.id,
    domain,
    quantity,
    ...
    backupEnergyWh,
    backupEnergyKWh: backupEnergyWh / 1000,
  };
}
...
function buildAutonomySnapshot({ domain, method, manualHours, autonomyHours, autonomyDays, forceAutonomyBattery }) {
  ...
  const hours = domain === "emergency"
    ? clampEmergencyBackupHours(autonomyHours, EMERGENCY_DEFAULT_BACKUP_HOURS)
    : Math.max(0, toNumber(autonomyHours, 0));
  const days = domain === "emergency" ? 0 : Math.max(0, toNumber(autonomyDays, 0));
  const totalHours = Number((domain === "emergency" ? hours : Math.max(hours, days * 24)).toFixed(2));
...
function mergeItemWithOverride(item, override = {}, domain = "solar") {
  return {
    ...item,
    ...override,
    quantity: Number(override.quantity ?? item.quantity ?? 1) || 1,
    usageHoursPerDay: domain === "emergency" ? EMERGENCY_BASE_LOAD_HOURS : (Number(override.usageHoursPerDay ?? item.usageHoursPerDay ?? 0) || 0),
    hasSoftStarter: Boolean(override.hasSoftStarter ?? item.hasSoftStarter ?? false),
  };
}

function matchScenarioEquipmentItems(scenario, equipmentLibrary) {
  ...
  for (const label of wanted) {
    const exact = equipmentLibrary.find((item) =>
      !used.has(item.id) && (item.title.includes(label) || item.category.includes(label))
    );
    const fallback = equipmentLibrary.find((item) =>
  ...
  if (!picked.length && scenario.domain === "solar") {
    return equipmentLibrary.filter((item) => ["eq-001", "eq-003", "eq-022"].includes(item.id));
  }
...
export default function CalculationInputs() {
  ...
  const [equipmentLibrary, setEquipmentLibrary] = React.useState(() => getConsumerEquipmentLibrary());
  React.useEffect(() => {
    let active = true;
    loadConsumerEquipmentLibrary().then((list) => { if (active && Array.isArray(list)) setEquipmentLibrary(list); });
    return () => { active = false; };
  }, []);
  ...
  const scenarioSeededItems = React.useMemo(() => matchScenarioEquipmentItems(scenario, equipmentLibrary), [scenario, equipmentLibrary]);
  ...
  const groupedEquipmentItems = React.useMemo(() => {
    const groups = new Map(CONSUMER_CATEGORY_ORDER.map((category) => [category, []]));
    items.forEach((item) => {
      const category = inferConsumerCategory(item);
  ...
  const selectedItems = React.useMemo(() => {
    return equipmentLibrary
      ...
      .map((item) => mergeItemWithOverride(item, itemOverrides[item.id], domain));
  }, [selectedIds, itemOverrides, equipmentLibrary]);
  ...
  }, [domain, method, scenario, environment, environmentAssessment, selectedItems, manualEnergyKWh, manualPowerW, manualCurrentA, manualVoltage, manualHours, autonomyHours, panelPowerW, panelCount, psh, acVoltageRoute, totalPanelPowerW, calculatedPvDailyKWh, profileVoltage, profileTotalEnergyWh, profilePeakPowerW, profileSurgePowerW, profileLoadProfile]);
  ...
  const equipmentTraces = React.useMemo(() => {
    ...
    const backupHours = equipmentDomain === "emergency" ? clampEmergencyBackupHours(autonomyHours) : 0;
  ...
  const equipmentStats = React.useMemo(() => equipmentTraces.reduce((acc, trace) => {
    ...
    acc.backupEnergyKWh += trace.backupEnergyKWh || trace.energyKWh;
    ...
    return acc;
  }, { selectedCount: 0, ratedPowerW: 0, totalPowerW: 0, totalEnergyKWh: 0, backupEnergyKWh: 0, acCurrentA: 0, startCurrentA: 0, surgePowerW: 0, motorCount: 0, softStarterCount: 0 }), [equipmentTraces]);
  ...
  const buildSystemSetupHandoff = (finalResult) => {
    ...
    const dailyEnergyKWh = Number(toNumber(
      method === "equipment" ? (effectiveDomain === "emergency" ? equipmentStats.backupEnergyKWh : equipmentStats.totalEnergyKWh) :
  ...
  const confirmLoad = () => {
    ...
    const finalResult = method === "equipment" ? {
      ...
      totalEnergyKWh: effectiveDomain === "emergency" ? (equipmentStats.backupEnergyKWh || result.totalEnergyKWh) : (equipmentStats.totalEnergyKWh || result.totalEnergyKWh),
      baseLoadEnergyKWh: effectiveDomain === "emergency" ? equipmentStats.totalEnergyKWh : undefined,
      backupHours: effectiveDomain === "emergency" ? clampEmergencyBackupHours(autonomyHours) : undefined,
      ...
      <div id="shil-calculation-inputs-root" className="shil-equipment-page shil-calculation-inputs-page">
        <section className="shil-env-card">
          ...
          {isReadyScenarioEquipmentFlow ? (
            <p className="shil-muted-note">{domain === "emergency" ? "سناریوی آماده برق اضطراری" : "سناریوی آماده خورشیدی"} بعد از تأیید شرایط محیطی وارد این لیست شده است. تجهیزات پیشنهادی سناریو از قبل انتخاب شده‌اند؛ می‌توانی حذف، اضافه یا تعداد را اصلاح کنی و بعد مسیر اصلی پروژه با موتور محاسباتی همان شاخه ادامه پیدا می‌کند. در برق اضطراری ساعت مبنای همه تجهیزات ثابت و برابر 1 ساعت است.</p>
          ) : null}
        ...
        {method === "equipment" ? (
          <section className="shil-env-card shil-equipment-picker-card">
            ...
            <button type="button" className="shil-equipment-field" onClick={() => setIsEquipmentPickerOpen((v) => !v)}>
              <span>{isReadyScenarioEquipmentFlow ? "اصلاح تجهیزات سناریوی انتخابی" : `انتخاب از بانک ${items.length} تجهیز`}</span>
            ...
            {isEquipmentPickerOpen ? (
              <div className="shil-equipment-picker-panel">
                ...
                />
                <div className="shil-equipment-scroll-list shil-equipment-category-list">
                  {groupedEquipmentItems.map(([categoryName, categoryItems]) => (
                    <details className="shil-equipment-category" key={categoryName}>
                      <summary><strong>{categoryName}</strong><span>{categoryItems.length} تجهیز</span></summary>
                      <div className="shil-equipment-category-items">
                        {categoryItems.map((item) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span>{item.ratedPowerW} W · {effectiveDomain === "emergency" ? EMERGENCY_BASE_LOAD_HOURS : item.usageHoursPerDay} h · {isMotor ? "موتوری" : "مصرفی"}</span>
                            </button>
                          );
                        })}
                      </div>
              ...
              return (
                <article key={item.id} className="shil-equipment-card active">
                  ...
                  <div className="shil-equipment-controls">
                    ...
                    <label>{effectiveDomain === "emergency" ? "ساعت مبنای تجهیز" : "ساعت مصرف"}<input className="shil-input" value={effectiveDomain === "emergency" ? EMERGENCY_BASE_LOAD_HOURS : (override.usageHoursPerDay ?? item.usageHoursPerDay)} onChange={effectiveDomain === "emergency" ? undefined : (e) => patchOverride(item.id, { usageHoursPerDay: e.target.value })} readOnly={effectiveDomain === "emergency"} aria-readonly={effectiveDomain === "emergency" ? "true" : undefined} inputMode="decimal" /></label>
                    {showExpert ? (
                      <>
                    ...
                    <details className={`shil-equipment-calc-accordion ${effectiveDomain === "emergency" ? "is-emergency" : "is-solar"}`}>
                      ...
                      <div className="shil-equipment-calc-domain-note">
                        {effectiveDomain === "emergency"
                          ? <span dir="rtl">منطق برق اضطراری · ساعت مبنای تجهیز ۱ ساعت است؛ انرژی ذخیره برای <bdi dir="ltr" className="shil-ltr-value">{enNumber(trace.backupHours || 3, 1)}</bdi> ساعت پشتیبانی محاسبه می‌شود.</span>
                      ...
                      <div className="shil-equipment-calc-grid">
                        ...
                        <div><span>{effectiveDomain === "emergency" ? "ساعت مبنای تجهیز" : "ساعت مصرف ثبت‌شده"}</span><strong>{enValue(trace.usageHoursPerDay, "h", 1)}</strong></div>
        ...
        {method === "equipment" ? (
          <section className="shil-env-card shil-equipment-results-card">
            ...
            <div className="shil-summary-grid">
              ...
              <div><span>{effectiveDomain === "emergency" ? "انرژی مبنای 1 ساعت" : "انرژی روزانه"}</span><strong>{enValue(equipmentStats.totalEnergyKWh || enginePreview.baseLoadEnergyWh / 1000 || enginePreview.totalEnergyKWh, "kWh", 2)}</strong></div>
              {effectiveDomain === "emergency" ? <div><span>انرژی ذخیره موردنیاز</span><strong>{enValue(equipmentStats.backupEnergyKWh || enginePreview.backupEnergyWh / 1000 || enginePreview.totalEnergyKWh, "kWh", 2)}</strong></div> : null}
