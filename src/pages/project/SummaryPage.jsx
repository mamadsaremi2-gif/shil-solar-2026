...
function SummaryGrid({ rows = [] }) {
  ...
  const groups = [];
  for (let index = 0; index < paddedRows.length; index += 4) {
    groups.push(paddedRows.slice(index, index + 4));
  }
  ...
  return (
    <div className="shil-summary-data" data-keep-card="true">
      <table className="shil-compact-summary-table" aria-label="اطلاعات چکیده">
        <tbody>
          {compactGroups.map((group, groupIndex) => (
            <React.Fragment key={`summary-group-${groupIndex}`}>
              <tr>
                {group.map(([label], index) => (
                  <th key={`h-${groupIndex}-${index}`} scope="col">{label}</th>
                ))}
              </tr>
              <tr>
                {group.map(([label, value], index) => (
                  <td key={`v-${groupIndex}-${index}`} title={`${label}: ${value || "-"}`}>
                    {value || "-"}
                  </td>
                ))}
              </tr>
      ...
      {cleanRows.length > 8 ? (
        <>
          ...
          <div className={open ? "shil-summary-accordion open" : "shil-summary-accordion"}>
            <table className="shil-compact-summary-table shil-summary-detail-table" aria-label="جزئیات چکیده">
              <tbody>
                {detailGroups.map((group, groupIndex) => (
                  <React.Fragment key={`detail-group-${groupIndex}`}>
                    <tr>
                      {group.map(([label], index) => (
                        <th key={`dh-${groupIndex}-${index}`} scope="col">{label}</th>
                      ))}
                    </tr>
                    <tr>
                      {group.map(([label, value], index) => (
                        <td key={`dv-${groupIndex}-${index}`} title={`${label}: ${value || "-"}`}>
                          {value || "-"}
                        </td>
                      ))}
                    </tr>
