...
export default function SummaryPage() {
  ...
  const applyAiPreview = () => {
    ...
    const previewPayload = {
      createdAt: new Date().toISOString(), mode: "optional-ai-installation-visualization", installMode,
            ...
            {aiOpen ? (
              <div className="shil-ai-preview-body">
                ...
                <div className="shil-ai-preview-layout shil-ai-install-preview-layout">
                  <div className={aiApplied ? "shil-ai-preview-visual ready shil-ai-install-visual" : "shil-ai-preview-visual shil-ai-install-visual"}>{imageTransferred && environmentImage?.src ? <img src={environmentImage.src} alt="تصویر محل نصب پروژه" /> : null}<div className="shil-ai-sky" /><div className="shil-ai-roof">{Array.from({ length: Math.min(Number(panelCount) || 6, 12) }).map((_, index) => <span key={index} />)}</div><strong>{aiApplied ? "تصویر مجازی نصب آماده شد" : imageTransferred ? "تصویر منتقل شد؛ آماده اعمال" : "در انتظار افزودن تصویر محل نصب"}</strong></div>
                ...
                <div className="shil-action-row shil-ai-apply-row"><button type="button" className="shil-primary-small" onClick={applyAiPreview}>اعمال شبیه‌سازی هوش مصنوعی</button><span className="shil-muted-line">این مرحله اختیاری است و مانع ادامه پروژه نمی‌شود.</span></div>
                {aiMessage ? <div className={aiApplied ? "shil-inline-success" : "shil-inline-warning"}>{aiMessage}</div> : null}
                {aiApplied ? <div className="shil-ai-engineering-output"><AiReasonTable title="جدول مهندسی پنل خورشیدی" rows={panelRows} /><AiReasonTable title="جدول مهندسی اینورتر" rows={inverterRows} /><AiReasonTable title="جدول مهندسی باتری" rows={batteryRows} /><div className="shil-ai-final-note"><strong>نتیجه شبیه‌سازی</strong><p>تصویر مجازی بر اساس تصویر محل نصب، نوع چیدمان انتخابی و دیتای واقعی موتور محاسبات آماده شده است.</p><button type="button" className="shil-soft-button" onClick={confirmAiPreview}>تایید خروجی هوش مصنوعی</button></div></div> : null}
              </div>
            ) : null}
          </div>
        ) : null}
