...
function AdminLoginPanel({ onBack }) {
  ...
  async function handleSubmit(event) {
    ...
    setMessage(result.ok ? 'ورود مدیریت انجام شد.' : result.error || 'ورود ناموفق بود.');
  }

  return (
    <div className="shell admin-login-shell">
      <section className="panel auth-card admin-login-card">
        <div className="auth-brand">
          <strong>ورود به پنل مدیریت SHIL</strong>
          <span>{isConfigured ? 'با حساب تأییدشده مدیر وارد شوید.' : 'حالت محلی فعال است؛ هر ایمیل و رمز معتبر وارد کنید.'}</span>
        </div>
        <form className="auth-form" onSubmit={handleSubmit}>
          <label>ایمیل<input type="email" value={form.email} onChange={(e) => setForm((prev) => ({ ...prev, email: e.target.value }))} required /></label>
          <label>رمز عبور<input type="password" value={form.password} onChange={(e) => setForm((prev) => ({ ...prev, password: e.target.value }))} required minLength={3} /></label>
          <button className="btn btn--primary" type="submit">ورود به پنل مدیریت</button>
          <button className="btn btn--ghost" type="button" onClick={onBack}>بازگشت به داشبورد</button>
...
export function AdminPage() {
  ...
  const { profile, isAdmin, user, signIn, signOut, isConfigured } = useAuth();
  ...
  if (!isAdmin) {
    return <AdminLoginPanel onBack={goDashboard} />;
  }
  ...
  return (
    <div className="shell">
      <header className="topbar">
        ...
        <button className="btn btn--secondary" type="button" onClick={loadAdminData}>به‌روزرسانی</button><button className="btn btn--ghost" type="button" onClick={signOut}>خروج</button>
      </header>
