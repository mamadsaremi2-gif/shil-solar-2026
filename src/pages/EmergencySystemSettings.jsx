...
function buildDesign({handoff,backupHours,reserveFactor,dodPercent,cableLength,cableLengthFactor,inverterId,batteryId,banks}){
 ...
 const hours=clampEmergencyBackupHours(backupHours,EMERGENCY_DEFAULT_BACKUP_HOURS); const reserve=Math.max(1,number(reserveFactor,1.25)); const dod=Math.min(.95,Math.max(.2,number(dodPercent,80)/100)); const inverterEfficiency=.92;
...
export default function EmergencySystemSettings(){
 ...
 const confirmedInputHours=number(handoff?.autonomy?.backupHours,handoff?.autonomy?.hours); const inheritedHours=clampEmergencyBackupHours(confirmedInputHours||EMERGENCY_DEFAULT_BACKUP_HOURS);
 ...
 const invRows=[["مدل",label(design.inverter)],["توان طراحی",`${fmt(design.inverter.designPowerW)} W`],["توان نامی",`${fmt(design.inverter.ratedPowerW||design.inverter.powerW)} W`],["ولتاژ بانک",`${fmt(design.inverter.dcVoltage||design.inverter.batteryVoltage)} VDC`],["جریان DC",`${fmt(design.electrical.estimatedDcCurrentA,1)} A`],["نوع انتخاب",design.inverter.selectionMode==="manual"?"دستی":"خودکار"]];
...
<SettingsField labelText="ساعت مبنای تجهیزات" value={EMERGENCY_BASE_LOAD_HOURS} readOnly note="برای همه بارهای برق اضطراری ثابت است"/><SettingsField labelText="زمان پشتیبانی هدف" value={backupHours} readOnly note="از ورودی محاسبات؛ بازه مجاز 1 تا 24 ساعت"/>
