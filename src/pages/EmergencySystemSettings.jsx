...
function EquipmentCard({title,kind,items,value,onChange,item,rows}){
 ...
 return <section className="shil-emergency-equipment-block">
  ...
  <details className="shil-compact-details"><summary>مشاهده جزئیات محاسبات</summary><div className="shil-compact-data-grid">{rows.map(([k,v])=><div key={k}><span>{k}</span><strong>{v}</strong></div>)}</div></details>
 </section>;
}
...
function ResultCard({labelText,children}){return <article className="shil-summary-kv-card"><span className="shil-summary-kv-label">{labelText}</span><strong className="shil-summary-kv-value">{children}</strong></article>}
