...
export default function LoginPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (rememberLogin) {
        localStorage.setItem("shil:remember-login", "1");
        localStorage.setItem("shil:last-login-email", email);
      } else {
        localStorage.removeItem("shil:remember-login");
        localStorage.removeItem("shil:last-login-email");
      }
    ...
    <div className="shil-auth-page" dir="rtl" style={{ "--shil-login-bg": `url(${loginBackground})` }}>
      ...
      <section className="shil-auth-card">
        ...
        <form className="shil-auth-form" onSubmit={handleSubmit}>
          ...
          <input id="shil-login-password" name="password" type="password" value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="رمز عبور" autoComplete="current-password" dir="ltr"/>
