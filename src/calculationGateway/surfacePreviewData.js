...
function buildLoadMetrics(input = {}) {
  ...
  const itemEnergy = selectedItems.reduce((sum, item) => {
    const effectivePower = n(item.ratedPowerW || item.powerW, 0) * n(item.quantity, 1) * n(item.simultaneityFactor ?? item.diversityFactor, 1);
    const hours = domain === "emergency" ? EMERGENCY_BASE_LOAD_HOURS : n(item.usageHoursPerDay, 0);
    return sum + effectivePower * hours;
  }, 0);
  ...
  return {
    ...
    backupEnergyWh: domain === "emergency" ? totalEnergyWh : null,
  };
}
