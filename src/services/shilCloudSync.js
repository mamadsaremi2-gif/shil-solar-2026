...
export async function readCloudRecords(baseKeys = ["shil-feedback", "shil-assistant-questions", "shil-projects"]) {
  ...
  assertSessionPermission(PERMISSIONS.CLOUD_ADMIN, getCurrentSession());
  const { data, error } = await supabase
    .from(SHIL_CLOUD_TABLE)
    .select("*")
...
export async function readCloudAdminAuditLog(limit = 100) {
  if (!isSupabaseReady()) return [];
  assertSessionPermission(PERMISSIONS.AUDIT_READ, getCurrentSession());
  const { data, error } = await supabase
    .from("shil_admin_audit_log")
    .select("id,actor_user_id,actor_auth_id,action,entity,payload,created_at")
