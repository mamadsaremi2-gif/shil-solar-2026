import { writeAdminAuditLog } from "../services/shilCloudSync.js";
import { getCurrentSession } from "../auth/session.js";
import { getCachedRuntimeData, RUNTIME_KEYS, saveRuntimeAppData } from "../services/runtimeAppDataService.js";
...
export const ADMIN_SYSTEM_VERSION = "SHIL_AUTH_SECURITY_V3_4_ADMIN_RECOVERY_20260811";
...
export const DEFAULT_ENGINEERING_RULES = {
  ...
  emergency: {
    safetyFactor: 1.25, startupMarginFactor: 1.2, minimumBackupHours: 1, reservePct: 15,
  },
  protection: {
...
const DEFAULT_SECURITY = {
  ...
  pinVersion: 2,
  lastVerifiedAt: "",
  lastSensitiveVerifiedAt: "",
  sessionMinutes: 60,
  sensitiveSessionMinutes: 5,
};
...
async function derivePinHash(pin, salt, iterations = 210000) {
  ...
  const key = await crypto.subtle.importKey("raw", encoder.encode(normalizeDigits(pin).trim()), "PBKDF2", false, ["deriveBits"]);
...
export function saveAdminProjectPathCards(cards) {
  ...
  const saved = write(ADMIN_KEYS.projectPathCards, clean);
  saveRuntimeAppData(RUNTIME_KEYS.projectPathCards, saved).catch((error) => console.warn("Project cards cloud sync:", error?.message || error));
  return saved;
}
...
export function readAdminCatalog() {
  const localCatalog = read(ADMIN_KEYS.equipmentCatalog, DEFAULT_EQUIPMENT_CATALOG);
  const runtimeCatalog = getCachedRuntimeData(RUNTIME_KEYS.equipmentCatalog, null);
  return { ...DEFAULT_EQUIPMENT_CATALOG, ...(localCatalog || {}), ...(runtimeCatalog || {}) };
}
...
export function saveAdminCatalog(catalog) {
  ...
  const saved = write(ADMIN_KEYS.equipmentCatalog, merged);
  saveRuntimeAppData(RUNTIME_KEYS.equipmentCatalog, saved).catch((error) => console.warn("Equipment cloud sync:", error?.message || error));
  return saved;
}

export function readAdminDefaults() {
  const localDefaults = read(ADMIN_KEYS.defaults, DEFAULT_ADMIN_DEFAULTS);
  const runtimeDefaults = getCachedRuntimeData(RUNTIME_KEYS.adminDefaults, null);
  return { ...DEFAULT_ADMIN_DEFAULTS, ...localDefaults, ...(runtimeDefaults || {}) };
}
...
export function saveAdminDefaults(defaults) {
  ...
  const saved = write(ADMIN_KEYS.defaults, merged);
  saveRuntimeAppData(RUNTIME_KEYS.adminDefaults, saved).catch((error) => console.warn("Admin defaults cloud sync:", error?.message || error));
  return saved;
}
...
export function logAdminAction(type, payload = {}) {
  ...
  const safePayload = payload && typeof payload === "object"
    ...
    : {};
  const next = [{ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, type, payload: safePayload, at: new Date().toISOString(), version: ADMIN_SYSTEM_VERSION }, ...current].slice(0, 40);
  try { write(ADMIN_KEYS.auditLog, next); }
  catch (error) {
    try { write(ADMIN_KEYS.auditLog, next.slice(0, 10)); }
    catch { window.localStorage.removeItem(ADMIN_KEYS.auditLog); }
  }
  return next;
}
...
export function isAdminPinVerified() {
  ...
  if (!security.pinConfigured || !security.pinHash || !security.pinSalt) return false;
  const last = security.lastVerifiedAt ? new Date(security.lastVerifiedAt).getTime() : 0;
  return Boolean(last && Date.now() - last < Number(security.sessionMinutes || 60) * 60 * 1000);
}

export async function verifyAdminPin(pin) {
  const security = readAdminSecurity();
  if (!security.pinConfigured || !security.pinHash || !security.pinSalt || Number(security.pinVersion || 0) < 2) {
    logAdminAction("admin-security:pin-unconfigured", {});
    return false;
  }
  const actual = await derivePinHash(pin, base64ToBytes(security.pinSalt), Number(security.pinIterations || 210000));
  const valid = constantTimeEqual(actual, base64ToBytes(security.pinHash));
  if (!valid) {
    logAdminAction("admin-security:pin-failed", {});
    return false;
  }
  write(ADMIN_KEYS.security, { ...security, lastVerifiedAt: new Date().toISOString() });
  logAdminAction("admin-security:pin-verified", {});
  return true;
}


export function isAdminSensitiveVerified() {
  const security = readAdminSecurity();
  const last = security.lastSensitiveVerifiedAt ? new Date(security.lastSensitiveVerifiedAt).getTime() : 0;
  return Boolean(last && Date.now() - last < Number(security.sensitiveSessionMinutes || 5) * 60 * 1000);
}

export async function verifyAdminSensitivePin(pin) {
  const security = readAdminSecurity();
  if (!security.pinConfigured || !security.pinHash || !security.pinSalt) return false;
  const actual = await derivePinHash(pin, base64ToBytes(security.pinSalt), Number(security.pinIterations || 210000));
  const valid = constantTimeEqual(actual, base64ToBytes(security.pinHash));
  if (!valid) {
    logAdminAction("admin-security:sensitive-pin-failed", {});
    return false;
  }
  write(ADMIN_KEYS.security, { ...security, lastVerifiedAt: new Date().toISOString(), lastSensitiveVerifiedAt: new Date().toISOString() });
  logAdminAction("admin-security:sensitive-pin-verified", {});
  return true;
}

export async function changeAdminPin(nextPin) {
  const clean = normalizeDigits(nextPin).trim();
  if (clean.length < 6) throw new Error("رمز لایه دوم ادمین باید حداقل ۶ رقم باشد.");
  const salt = crypto.getRandomValues(new Uint8Array(16));
  const digest = await derivePinHash(clean, salt, 210000);
  const security = readAdminSecurity();
  write(ADMIN_KEYS.security, {
    ...security,
    pinConfigured: true,
    pinHash: bytesToBase64(digest),
