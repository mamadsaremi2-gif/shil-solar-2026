...
function addEngineeringGateChecks(messages, input, loadResult, batteryResult, pvResult, inverterResult, controllerResult, cablingResult, protectionResult, simulationResult, industrialResult) {
  ...
  if (systemIsBatteryBased) {
    ...
    if (batteryResult && batteryResult.voltageCompatible === false) {
      pushMessage(messages, "error", "کنترل ولتاژ باتری و اینورتر", voltageCompatibilityMessage(input.systemVoltage, input.batteryUnitVoltage), "system");
    }
  ...
  if (hasPv) {
    ...
    if (!pvResult.mpptWindowOk || !controllerResult?.mpptWindowOk || pvResult.mpptHotWindowOk === false) {
      ...
      pushMessage(messages, "warning", "کنترل مهندسی MPPT: ولتاژ کاری نامناسب", `Vmp رشته در شرایط نامی ${round(pvResult.stringVmp, 0)}V و در گرمای محیط حدود ${round(pvResult.stringVmpHot, 0)}V است؛ بازه مطلوب با این پنل تقریباً ${minSeries} تا ${maxSeries} پنل سری است.`, "system");
    }
    ...
    if (pvResult.mpptCurrentOk === false) {
      pushMessage(messages, "error", "کنترل جریان ورودی MPPT", `جریان ورودی رشته‌ها حدود ${round(pvResult.stringCurrentA, 1)}A است و از حد ${input.mpptMaxInputCurrent}A بیشتر می‌شود. تعداد رشته موازی را کم کن یا MPPT/اینورتر با جریان ورودی بالاتر انتخاب کن.`, "system");
    }
...
export function generateAdvisorMessages(input, loadResult, batteryResult, pvResult, inverterResult, controllerResult, cablingResult, protectionResult, simulationResult, industrialResult, installationResult) {
  const messages = [];
  ...
  if (installationResult) {
    if (installationResult.area.status === "error") {
      pushMessage(messages, "error", "فضای نصب پنل کافی نیست", `برای ${pvResult.panelCount} پنل حدود ${installationResult.area.requiredAreaM2} مترمربع فضای اجرایی با مسیر سرویس لازم است؛ فضای مفید واردشده ${installationResult.area.usableAreaM2} مترمربع است.`, "site");
    } else if (installationResult.area.status === "warning") {
      pushMessage(messages, "warning", "فضای نصب مرزی است", `فضای مفید نصب به فضای موردنیاز نزدیک است. مسیر دسترسی ${installationResult.area.maintenanceWalkwayM}m و فاصله از لبه بام باید در بازدید محل کنترل شود.`, "site");
    }
    if (installationResult.orientation.status !== "pass") {
      pushMessage(messages, installationResult.orientation.status === "error" ? "error" : "warning", "جهت نصب پنل نیازمند بازبینی است", `جهت فعلی ${installationResult.orientation.label} با اتلاف تقریبی ${installationResult.orientation.orientationLossPercent}% محاسبه شد. جهت پیشنهادی برای ایران عموماً جنوب است.`, "site");
    }
    if (installationResult.tilt.status !== "pass") {
      pushMessage(messages, "warning", "زاویه نصب بهینه نیست", `زاویه انتخابی ${installationResult.tilt.selectedTiltDeg}° است؛ برای هدف ${installationResult.tilt.designGoal} زاویه پیشنهادی حدود ${installationResult.tilt.recommendedTiltDeg}° و اتلاف زاویه حدود ${installationResult.tilt.tiltLossPercent}% است.`, "site");
    }
    if (installationResult.cableRoute.routeStatus !== "pass") {
      pushMessage(messages, installationResult.cableRoute.routeStatus === "error" ? "warning" : "info", "مسیر کابل DC طولانی است", `طول یک‌طرفه مسیر DC ${installationResult.cableRoute.dcCableOneWayLengthM}m است؛ افت ولتاژ ${installationResult.cableRoute.voltageDropPercent}% و کابل پیشنهادی ${installationResult.cableRoute.recommendedCableMm2}mm² محاسبه شد. نزدیک‌تر کردن اینورتر به آرایه بررسی شود.`, "system");
    }
    if (installationResult.losses.totalInstallationLossPercent > 18) {
      pushMessage(messages, "warning", "تلفات اجرایی PV بالاست", `مجموع تلفات نصب/محیطی حدود ${installationResult.losses.totalInstallationLossPercent}% است و تولید خالص روزانه به حدود ${installationResult.losses.netDailyProductionWh}Wh می‌رسد.`, "site");
    }
    if (installationResult.protection.stringFuseRequired) {
      pushMessage(messages, "info", "فیوز رشته و Combiner لازم است", `به دلیل وجود رشته‌های موازی، ${installationResult.protection.stringFuse} و ${installationResult.protection.combinerBox} پیشنهاد شد.`, "system");
    }
    pushMessage(messages, "info", "رفتار تولید در ساعات غیر PSH", installationResult.daytimeEquivalent.nonPshBehavior, "review");
  }
