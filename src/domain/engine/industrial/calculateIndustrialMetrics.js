...
export function calculateIndustrialMetrics(input, loads, battery, pv, inverter, controller, cabling, protection, simulation, installation) {
  ...
  const installationNetDailyProductionWh = installation?.losses?.netDailyProductionWh ?? (pv?.estimatedDailyProductionWh || 0);
  const installationNetCoverageRatio = pv && loads.totalDailyEnergyWh > 0 ? installationNetDailyProductionWh / loads.totalDailyEnergyWh : 0;
  const daytimeLoadWh = loads.totalDailyEnergyWh * 0.45;
  const nightLoadWh = loads.totalDailyEnergyWh * 0.55;
  const directSolarWh = pv ? Math.min(installationNetDailyProductionWh, daytimeLoadWh) : 0;
  const pvSurplusAfterDayLoadWh = pv ? Math.max(installationNetDailyProductionWh - directSolarWh, 0) : 0;
  const batteryChargeAvailableWh = pv ? pvSurplusAfterDayLoadWh * Math.max(input.batteryRoundTripEfficiency || 0.9, 0.5) : 0;
  const nightCoveredByBatteryWh = input.systemType !== 'gridtie' ? Math.min(nightLoadWh, Math.max(batteryChargeAvailableWh, battery.usableBatteryWh || 0)) : 0;
  const energyNotServedWh = Math.max(loads.totalDailyEnergyWh - directSolarWh - nightCoveredByBatteryWh, 0);
  const pvCurtailmentWh = pv ? Math.max(pvSurplusAfterDayLoadWh - nightCoveredByBatteryWh / Math.max(input.batteryRoundTripEfficiency || 0.9, 0.5), 0) : 0;
  const selfConsumptionRatio = pv && installationNetDailyProductionWh > 0 ? (directSolarWh + Math.min(pvSurplusAfterDayLoadWh, nightCoveredByBatteryWh / Math.max(input.batteryRoundTripEfficiency || 0.9, 0.5))) / installationNetDailyProductionWh : 0;
  ...
  const serviceabilityScore = Math.max(0, Math.min(100,
    ...
    - (pv && installationNetCoverageRatio < pvCoverageRatio * 0.9 ? 7 : 0)
  ...
  if (pv && installation && installationNetCoverageRatio < pvCoverageRatio * 0.9) {
    actionItems.push(`با لحاظ شرایط نصب، تولید خالص به حدود ${round(installationNetDailyProductionWh)}Wh/day می‌رسد؛ جهت، زاویه، سایه، گردوغبار و مسیر کابل باید اصلاح شوند.`);
  }
  ...
  return {
    ...
    installationNetDailyProductionWh: round(installationNetDailyProductionWh),
    installationNetCoverageRatio: round(installationNetCoverageRatio, 2),
    energyBalance: { daytimeLoadWh: round(daytimeLoadWh), nightLoadWh: round(nightLoadWh), directSolarWh: round(directSolarWh), pvSurplusAfterDayLoadWh: round(pvSurplusAfterDayLoadWh), batteryChargeAvailableWh: round(batteryChargeAvailableWh), nightCoveredByBatteryWh: round(nightCoveredByBatteryWh), energyNotServedWh: round(energyNotServedWh), pvCurtailmentWh: round(pvCurtailmentWh), selfConsumptionRatio: round(selfConsumptionRatio, 2), dailyCoveragePercent: round(((directSolarWh + nightCoveredByBatteryWh) / Math.max(loads.totalDailyEnergyWh, 1)) * 100, 0) },
