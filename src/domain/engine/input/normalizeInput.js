...
export function normalizeInput(form) {
  ...
  const siteSurvey = {
    ...
    compassAzimuthDeg: parseFaNumber(form.compassAzimuthDeg, parseFaNumber(form.cameraAzimuthDeg, parseFaNumber(form.azimuthDeg, 0))),
    cameraAzimuthDeg: parseFaNumber(form.cameraAzimuthDeg, parseFaNumber(form.compassAzimuthDeg, parseFaNumber(form.azimuthDeg, 0))),
    cameraTiltDeg: positive(form.cameraTiltDeg, positive(form.tiltAngle, 30)),
    cameraOrientationStatus: form.cameraOrientationStatus || "not_captured",
    tiltAngleDeg: positive(form.tiltAngle, positive(form.cameraTiltDeg, 30)),
    sitePhotoUrl: form.sitePhotoUrl || '',
    compassScreenshotUrl: form.compassScreenshotUrl || '',
    cameraCaptureAt: form.cameraCaptureAt || '',
  };
  const shadowObjects = Array.isArray(form.shadowObjects) ? form.shadowObjects.map((item, index) => ({
    ...
    shadowPercent: bounded(item.shadowPercent ?? item.manualShadowPercent ?? item.percentShadow, 0, 0, 95),
  })) : [];
  ...
  return {
    ...
    engineeringMode,
    siteSurvey,
    shadowObstacleHeightM: nonNegative(form.shadowObstacleHeightM, 0),
    shadowObstacleDistanceM: nonNegative(form.shadowObstacleDistanceM, 0),
    shadowObstacleDirectionDeg: parseFaNumber(form.shadowObstacleDirectionDeg, 180),
    shadowCriticalHours: form.shadowCriticalHours || '',
    shadowPercent: bounded(form.shadowPercent ?? form.manualShadowPercent, 0, 0, 95),
    manualShadowPercent: bounded(form.manualShadowPercent ?? form.shadowPercent, 0, 0, 95),
    climateDataSource: form.climateDataSource || 'offline_iran_climate_cache',
    climateProvider: form.climateProvider || form.climateDataSource || 'offline_iran_climate_cache',
    climateOnlineStatus: form.climateOnlineStatus || 'not_requested',
    climateFetchedAt: form.climateFetchedAt || '',
    climateFallbackReason: form.climateFallbackReason || '',
    climateCorrectionEnabled: Boolean(form.climateCorrectionEnabled),
    realPsh: nonNegative(form.realPsh, 0),
    realIrradianceKwhM2Day: nonNegative(form.realIrradianceKwhM2Day, 0),
    clearSkyIrradianceKwhM2Day: nonNegative(form.clearSkyIrradianceKwhM2Day, 0),
    realAverageTemperature: parseFaNumber(form.realAverageTemperature, NaN),
    realMinTemperature: parseFaNumber(form.realMinTemperature, NaN),
    realMaxTemperature: parseFaNumber(form.realMaxTemperature, NaN),
    autoTemperatureFactor: nonNegative(form.autoTemperatureFactor, 0),
    autoIrradianceFactor: nonNegative(form.autoIrradianceFactor, 0),
    autoDustFactor: nonNegative(form.autoDustFactor, 0),
    autoSeasonalFactor: nonNegative(form.autoSeasonalFactor, 0),
    environmentalAutoFactor: nonNegative(form.environmentalAutoFactor, 0),
    predictedDailyProductionKwh: nonNegative(form.predictedDailyProductionKwh, 0),
    predictedAnnualProductionKwh: nonNegative(form.predictedAnnualProductionKwh, 0),
    climateMonthlyPshJson: form.climateMonthlyPshJson || '',
    climateProductionForecastJson: form.climateProductionForecastJson || '',
    sitePhotoUrl: form.sitePhotoUrl || '',
    compassScreenshotUrl: form.compassScreenshotUrl || '',
    cameraCaptureAt: form.cameraCaptureAt || '',
    shadowObjects,
    ...
    azimuthDeg: parseFaNumber(form.azimuthDeg, parseFaNumber(form.cameraAzimuthDeg, 0)),
    cameraAzimuthDeg: parseFaNumber(form.cameraAzimuthDeg, parseFaNumber(form.azimuthDeg, 0)),
    cameraTiltDeg: positive(form.cameraTiltDeg, positive(form.tiltAngle, 30)),
    cameraOrientationStatus: form.cameraOrientationStatus || "not_captured",
    ...
    mismatchLossPercent: bounded(form.mismatchLossPercent, 2, 0, 8),
    mpptLossPercent: bounded(form.mpptLossPercent, 1.2, 0, 8),
    loadItems,
  };
}
