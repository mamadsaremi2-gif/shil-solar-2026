import { voltageCompatibilityMessage } from "../rules/engineeringRules.js";
...
export function evaluateDesignValidation(input, loads, battery, pv, inverter, controller, cabling, protection, simulation, industrial, installation) {
  const checks = [];
  const isBatteryBased = input.systemType !== 'gridtie';
  const hasPv = Boolean(pv);

  if (isBatteryBased) {
    if (battery?.voltageCompatible === false) {
      pushCheck(checks, { id: "battery-inverter-voltage-compatibility", category: "battery", severity: "error", status: "fail", title: "ولتاژ باتری و اینورتر همخوان نیست", message: voltageCompatibilityMessage(input.systemVoltage, input.batteryUnitVoltage), metric: `${battery.bankVoltage || 0}V`, limit: `${battery.voltageWindow?.min || "?"}-${battery.voltageWindow?.max || "?"}V`, recommendation: "باتری با ولتاژ نامی سازگار انتخاب کن یا ولتاژ سیستم/اینورتر را تغییر بده.", scoreImpact: 30 });
    }
  ...
  if (hasPv) {
    ...
    if (!pv.mpptWindowOk || pv.mpptHotWindowOk === false) {
      pushCheck(checks, { id: 'pv-mppt-window', category: 'pv', severity: 'warning', status: 'borderline', title: 'ولتاژ کاری رشته خارج از پنجره بهینه MPPT است', message: `Vmp رشته نامی ${round(pv.stringVmp, 0)}V و Vmp گرم ${round(pv.stringVmpHot, 0)}V است؛ پنجره MPPT ${input.mpptMinVoltage} تا ${input.mpptMaxVoltage}V است.`, metric: `${round(pv.stringVmp, 0)}V`, limit: `${input.mpptMinVoltage}-${input.mpptMaxVoltage}V`, recommendation: 'آرایش سری/موازی پنل یا مدل کنترلر را اصلاح کن.', scoreImpact: 8 });
    }

    if (pv.mpptCurrentOk === false) {
      pushCheck(checks, { id: "pv-mppt-input-current", category: "pv", severity: "error", status: "fail", title: "جریان ورودی MPPT بیش از حد مجاز است", message: `جریان آرایه حدود ${round(pv.stringCurrentA, 1)}A است و از حد ${input.mpptMaxInputCurrent}A بیشتر است.`, metric: `${round(pv.stringCurrentA, 1)}A`, limit: `${input.mpptMaxInputCurrent}A`, recommendation: "تعداد رشته موازی را کاهش بده یا اینورتر/MPPT با جریان ورودی بالاتر انتخاب کن.", scoreImpact: 25 });
    }
  ...
  if (installation) {
    if (installation.area.status === 'error') {
      pushCheck(checks, { id: 'installation-area-shortage', category: 'installation', severity: 'error', status: 'fail', title: 'فضای نصب آرایه PV کافی نیست', message: `فضای موردنیاز نصب ${round(installation.area.requiredAreaM2, 1)}m² و فضای مفید واردشده ${round(installation.area.usableAreaM2, 1)}m² است.`, metric: `${round(installation.area.usableAreaM2, 1)}m²`, limit: `${round(installation.area.requiredAreaM2, 1)}m²`, recommendation: 'تعداد پنل، جانمایی، سطح قابل استفاده یا محل نصب بازبینی شود.', scoreImpact: 28 });
    } else if (installation.area.status === 'warning') {
      pushCheck(checks, { id: 'installation-area-borderline', category: 'installation', severity: 'warning', status: 'borderline', title: 'فضای نصب آرایه PV مرزی است', message: `فضای مفید ${round(installation.area.usableAreaM2, 1)}m² به نیاز ${round(installation.area.requiredAreaM2, 1)}m² نزدیک است.`, metric: `${round(installation.area.usableAreaM2, 1)}m²`, limit: `${round(installation.area.requiredAreaM2, 1)}m²`, recommendation: 'مسیر سرویس، سایه جان‌پناه و فاصله از لبه بام در بازدید محل کنترل شود.', scoreImpact: 8 });
    }
    if (installation.orientation.status === 'error') {
      pushCheck(checks, { id: 'installation-orientation-poor', category: 'installation', severity: 'error', status: 'fail', title: 'جهت نصب برای تولید خورشیدی نامناسب است', message: `جهت فعلی ${installation.orientation.label} و اتلاف جهت حدود ${round(installation.orientation.orientationLossPercent, 1)}٪ است.`, metric: `${round(installation.orientation.orientationLossPercent, 1)}٪`, limit: 'ترجیحاً زیر 8٪', recommendation: 'پنل‌ها تا حد امکان به سمت جنوب یا جنوب‌شرق/جنوب‌غرب اصلاح شوند.', scoreImpact: 18 });
    } else if (installation.orientation.status === 'warning') {
      pushCheck(checks, { id: 'installation-orientation-borderline', category: 'installation', severity: 'warning', status: 'borderline', title: 'جهت نصب باعث افت تولید می‌شود', message: `اتلاف جهت حدود ${round(installation.orientation.orientationLossPercent, 1)}٪ است.`, metric: `${round(installation.orientation.orientationLossPercent, 1)}٪`, limit: 'ترجیحاً زیر 8٪', recommendation: 'در صورت محدودیت معماری، آرایه‌های جهت متفاوت روی MPPT جدا طراحی شوند.', scoreImpact: 7 });
    }
    if (installation.tilt.status !== 'pass') {
      pushCheck(checks, { id: 'installation-tilt-loss', category: 'installation', severity: 'warning', status: 'borderline', title: 'زاویه نصب از مقدار پیشنهادی فاصله دارد', message: `زاویه انتخابی ${round(installation.tilt.selectedTiltDeg, 0)}° و زاویه پیشنهادی ${round(installation.tilt.recommendedTiltDeg, 0)}° است.`, metric: `${round(installation.tilt.tiltLossPercent, 1)}٪`, limit: 'ترجیحاً زیر 6٪ افت', recommendation: 'زاویه سازه با هدف سالانه/زمستانی پروژه تطبیق داده شود.', scoreImpact: 6 });
    }
    if (installation.cableRoute.routeStatus === 'error') {
      pushCheck(checks, { id: 'installation-dc-route-long', category: 'installation', severity: 'warning', status: 'borderline', title: 'مسیر DC پنل تا اینورتر طولانی است', message: `طول یک‌طرفه DC ${round(installation.cableRoute.dcCableOneWayLengthM, 1)}m است.`, metric: `${round(installation.cableRoute.voltageDropPercent, 2)}٪ افت`, limit: `${round(input.dcVoltageDropLimit, 2)}٪`, recommendation: 'محل اینورتر/تابلو DC نزدیک‌تر، ولتاژ رشته بالاتر یا سطح مقطع بیشتر بررسی شود.', scoreImpact: 5 });
    }
    if (installation.losses.totalInstallationLossPercent > 20) {
      pushCheck(checks, { id: 'installation-losses-high', category: 'installation', severity: 'warning', status: 'borderline', title: 'تلفات محیطی و نصب بالاست', message: `تلفات نصب/محیطی حدود ${round(installation.losses.totalInstallationLossPercent, 1)}٪ و تولید خالص ${round(installation.losses.netDailyProductionWh, 0)}Wh/day است.`, metric: `${round(installation.losses.totalInstallationLossPercent, 1)}٪`, limit: 'پیشنهادی زیر 18٪', recommendation: 'سایه، گردوغبار، زاویه، جهت، مسیر کابل و برنامه شستشو اصلاح شود.', scoreImpact: 8 });
    }
  }
