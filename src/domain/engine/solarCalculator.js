...
import { calculateSolarSystem } from "../domain/engine/solarCalculator";
...
function buildSolarCalculatorLoads(loadItems = []) {
  return (loadItems || []).map((item) => ({
    name: item.name || "مصرف‌کننده",
    quantity: parseFaNumber(item.qty, 1),
    powerW: parseFaNumber(item.power, 0),
    currentA: parseFaNumber(item.current, 0),
    hours: parseFaNumber(item.hours, 0),
    powerFactor: parseFaNumber(item.powerFactor, 0.95),
    diversityFactor: parseFaNumber(item.coincidenceFactor, 1),
    startFactor: parseFaNumber(item.surgeFactor, 1),
    loadType: item.loadType || "mixed",
    description: item.description || item.note || "",
  })).filter((item) => item.quantity > 0 && item.powerW > 0);
}

function SolarCalculationPreview({ form, loadSummary }) {
  const result = useMemo(() => {
    const loads = buildSolarCalculatorLoads(form.loadItems);
    if (!loads.length) return null;

    return calculateSolarSystem(loads, {
      systemVoltage: parseFaNumber(form.systemVoltage, 48),
      sunHours: parseFaNumber(form.sunHours, 5),
      panelWatt: parseFaNumber(form.panelWatt, 550),
      autonomyDays: parseFaNumber(form.daysAutonomy, 1),
      batteryDod: parseFaNumber(form.dod, 0.8),
      batteryEfficiency: parseFaNumber(form.batteryRoundTripEfficiency, 0.9),
      systemEfficiency: parseFaNumber(form.panelLossFactor, 0.9) * parseFaNumber(form.controllerEfficiency, 0.96) * parseFaNumber(form.cableLossFactor, 0.97),
      inverterSafetyFactor: parseFaNumber(form.designFactor, 1.25),
    });
  }, [form]);
  ...
  return (
    <section className="panel panel--soft solar-live-preview">
      <div className="panel__header">
        <div>
          <h3>پیش‌نمایش زنده طراحی خورشیدی</h3>
          <p className="section-note">این خروجی از لیست مصرف‌کننده‌ها، ضریب همزمانی، ضریب راه‌اندازی، PF، تابش شهر و تنظیمات باتری/پنل ساخته می‌شود.</p>
        </div>
        <span className="badge">محاسبه خودکار</span>
      </div>

      <div className="metric-grid metric-grid--tight">
        <div className="metric-card metric-card--amber">
          <div className="metric-card__label">مصرف روزانه اصلاح‌شده</div>
          <div className="metric-card__value">{result.energy.adjustedDailyKwh} kWh</div>
        </div>
        <div className="metric-card metric-card--purple">
          <div className="metric-card__label">سانورتر پیشنهادی</div>
          <div className="metric-card__value">{result.inverter.recommendedInverterKw} kW</div>
        </div>
        <div className="metric-card metric-card--green">
          <div className="metric-card__label">باتری پیشنهادی</div>
          <div className="metric-card__value">{result.battery.nominalBatteryKwh} kWh</div>
        </div>
        <div className="metric-card metric-card--blue">
          <div className="metric-card__label">پنل پیشنهادی</div>
          <div className="metric-card__value">{result.pv.panelCount} × {result.pv.panelWatt}W</div>
        </div>
      </div>

      <div className="summary-list">
        <div><span>توان کارکرد واقعی</span><strong>{loadSummary.realRunPowerW.toFixed(0)} W</strong></div>
        <div><span>توان راه‌اندازی</span><strong>{result.inverter.maxSurgeW.toFixed(0)} W</strong></div>
        <div><span>آرایه پنل</span><strong>{result.summary.panels}</strong></div>
        <div><span>کنترلر</span><strong>{result.summary.controller}</strong></div>
      </div>
    </section>
  );
}
  ...
  if (form.calculationMode === "loads") {
    return (
      <div className="stack-lg">
        ...
        <SolarCalculationPreview form={form} loadSummary={loadSummary} />
