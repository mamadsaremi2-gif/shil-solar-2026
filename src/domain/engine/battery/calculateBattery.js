import { deriveBatterySeriesCount } from "../rules/engineeringRules.js";
...
function buildBackupBatteryScenarios(input, loadResult, roundTripEfficiency, thermalFactor, reserveFactor) {
  ...
  for (const systemVoltage of BACKUP_SYSTEM_VOLTAGES) {
    ...
    for (const batteryUnitVoltage of BACKUP_BATTERY_UNIT_VOLTAGES) {
      const series = deriveBatterySeriesCount(systemVoltage, batteryUnitVoltage);
      if (!series.ok) continue;
      const seriesCount = series.seriesCount;
...
export function calculateBattery(input, loadResult) {
  ...
  const voltageSeries = deriveBatterySeriesCount(input.systemVoltage, input.batteryUnitVoltage);
  const seriesCount = Math.max(1, voltageSeries.seriesCount);
  ...
  return {
    ...
    usableBatteryWh: round(usableBatteryWh),
    seriesCount,
    bankVoltage: round((input.batteryUnitVoltage || 0) * seriesCount, 1),
    voltageCompatible: voltageSeries.ok,
    voltageWindow: voltageSeries.window,
    parallelCount,
