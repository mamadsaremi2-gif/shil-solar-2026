...
export function FeedbackPage() {
  ...
  return (
    <main className="project-flow-shell feedback-page-v11" dir="rtl">
      <section className="flow-top-card compact-page-header">
        ...
        <div className="flow-top-title">
          <strong>اعلام نظر کاربران اپ</strong>
          <small>ارتباط مستقیم و خصوصی با مدیریت</small>
        </div>
        <span />
      </section>

      <section className="focus-content-card feedback-card-v11 feedback-card-final">
        <h2>نظرات و پیشنهادات شما موجب توسعه اپ و کیفیت محصولات و اضافه کردن تجهیزات جدید در سبد محصولات ما می‌گردد. لطفاً نظرات خود را برای ما ارسال کنید.</h2>
        <form className="feedback-form-v11" onSubmit={handleSubmit}>
          <label>
            موضوع نظر
            <select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })}>
              <option value="app-development">توسعه اپ</option>
              <option value="engineering-flow">فرآیند مهندسی</option>
              <option value="equipment-bank">بانک تجهیزات</option>
              <option value="products">محصولات و تجهیزات</option>
              <option value="bug-report">گزارش خطا</option>
          ...
          <label>
            ...
            <textarea
              ...
              placeholder="نظر، پیشنهاد، نیاز تجهیزاتی یا ایراد مشاهده‌شده را اینجا بنویسید..."
              rows={6}
            />
          </label>
          <div className="feedback-actions-v11">
            <button className="btn btn--primary" type="submit" disabled={saving}>{saving ? "در حال ارسال..." : "ارسال برای مدیریت"}</button>
        ...
        <section className="user-feedback-inbox" aria-label="کارتابل نظرات من">
          <div className="project-bucket-title"><strong>کارتابل نظرات من</strong><span>{myFeedback.length} مورد</span></div>
          {myFeedback.length ? (
            <div className="user-feedback-list">
              {myFeedback.map((item) => (
                <article key={item.id} className="user-feedback-item">
                  <div><strong>{item.category}</strong><small>{new Date(item.createdAt).toLocaleString("fa-IR")}</small></div>
                  <p>{item.message}</p>
                  <button type="button" className="btn btn--ghost btn--sm" onClick={() => hideForUser(item.id)}>حذف از کارتابل من</button>
                </article>
              ))}
            </div>
          ) : <p className="empty-state">هنوز نظری ثبت نکرده‌اید.</p>}
        </section>
      </section>
    </main>
  );
