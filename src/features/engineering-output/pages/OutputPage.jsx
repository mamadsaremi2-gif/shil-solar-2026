...
export function OutputPage() {
  ...
  if (!isBackup) {
    ...
    requiredEquipmentRows.splice(7, 0, ['فضای نصب پیشنهادی', `${formatNumber(installation?.area?.requiredAreaM2 || 0, 1)} m² با مسیر سرویس / وضعیت ${installation?.area?.status || '—'}`]);
    requiredEquipmentRows.splice(8, 0, ['حفاظت DC استاندارد', `${protection?.dcIsolatorRating || '—'} | ${protection?.dcSpdType || '—'} | IP ${protection?.enclosureIpRating || installation?.protection?.ipRating || '—'}`]);
  }
          ...
          {isBackup ? (
            ...
            <section className="panel panel--full">
              <div className="panel__header">
                ...
                <span className="badge">Installation Layer</span>
              </div>
              <div className="summary-list">
                <div><span>فضای پنل / فضای اجرایی لازم</span><strong>{formatNumber(installation.area.panelAreaM2, 1)} / {formatNumber(installation.area.requiredAreaM2, 1)} m²</strong></div>
                <div><span>فضای مفید واردشده</span><strong>{installation.area.usableAreaM2 ? `${formatNumber(installation.area.usableAreaM2, 1)} m²` : 'وارد نشده'}</strong></div>
                <div><span>مسیر سرویس / فاصله لبه</span><strong>{formatNumber(installation.area.maintenanceWalkwayM, 1)}m / {formatNumber(installation.area.edgeSetbackM, 1)}m</strong></div>
                <div><span>جهت نصب</span><strong>{installation.orientation.label} | افت {formatNumber(installation.orientation.orientationLossPercent, 1)}%</strong></div>
                <div><span>زاویه انتخابی / پیشنهادی</span><strong>{formatNumber(installation.tilt.selectedTiltDeg, 0)}° / {formatNumber(installation.tilt.recommendedTiltDeg, 0)}°</strong></div>
                <div><span>مسیر DC پنل تا اینورتر</span><strong>{formatNumber(installation.cableRoute.dcCableOneWayLengthM, 1)}m یک‌طرفه | افت {formatNumber(installation.cableRoute.voltageDropPercent, 2)}%</strong></div>
                <div><span>تلفات نصب و محیط</span><strong>{formatNumber(installation.losses.totalInstallationLossPercent, 1)}%</strong></div>
                <div><span>تولید خام PSH / تولید خالص</span><strong>{formatNumber(installation.daytimeEquivalent.grossWhAtPsh)} / {formatNumber(installation.daytimeEquivalent.netWhAtPsh)} Wh/day</strong></div>
                <div><span>استراتژی سری/موازی</span><strong>{installation.stringingStrategy.reason}</strong></div>
                <div><span>حفاظت DC</span><strong>{installation.protection.dcIsolator} | {installation.protection.dcSpd} | {installation.protection.ipRating}</strong></div>
              </div>
              <div className="advisor-list">
                <div className="advisor-card advisor-card--info"><strong>رفتار ۱۹ ساعت دیگر</strong><span>{installation.daytimeEquivalent.nonPshBehavior}</span></div>
                <div className="advisor-card advisor-card--info"><strong>لیست حفاظتی اجرا</strong><span>{installation.protection.combinerBox}، {installation.protection.stringFuse}، {installation.protection.earthing}</span></div>
              </div>
            </section>
          ) : null}
